...
var myHref = new Array('myHrefOrder_我的订单(卖家/买家)','myHrefGrxx_个人信息(卖家/买家)','myHrefDp_我的店铺','myHrefTeam_我的团队',
    ...
    'myAppDownload_APP下载数据查询','accountAndSecurity_账户与安全');
...
//               $(".tabslc").tabs();
        ...
        updateContent:function (sign) {// 提交方法sign 0 直接提交 1 为鼠标移出提交
            ...
            var content = $("#doc-wrap .demo").html();
                ...
                if(type=="head"){// 标题
                    ...
                    for(var i= 1 ;i<imgNum;i++){
                        ...
                        editHtml = "<tr><th>广告位编码：</th><td><input id='goodCodeInput' type='text' value='" + cmGoodCode + "' goodcodeinputsign='goodcodeinputsign' class='goodcodeinputsign' style='float:left;margin-bottom: 0.05rem;'></td></tr>";
		                    ...
		                    editHtml += " ><label class=\"jtlabel\">新页面</label></td></tr><tr class='jtggwtr allNeedShow showTr_3_all showTr_3_2' style='display:none;'><th>我的：</th><td><p class='selectType selectTypeMyHref'><select name='' id=''><option value='' bind=''></option>";
		                    // 我的数组
...
// 0.05rem;'></td></tr>";
  ...
  case '导购':
	  ...
	  break;
                    ...
                    var editPicImgSrc = p.find('.myview .goodsImage img').prop('src') == undefined?"":p.find('.myview .goodsImage img').prop('src');
...
editHtml += "<option value='decorateId_" + vm.aggregationList[j][0] + "' turnHref='" +  vm.aggregationList[j][2] + "?decorateId=" + vm.aggregationList[j][0] + "' bind='decorateId_" + vm.aggregationList[j][0] + "'>" + vm.aggregationList[j][1] + "</option>";
      ...
      //tab楼层配置展示
        ...
        tabmrsh =  $( "#mrsh" ),// 默认色号
        tabxzsh =  $( "#xzsh" ),// 选中色号
        tabLCTemplate = '<li class="swiper-slide kdli"> <a href="#{href}">#{label}</a> <img src="../../images/decorate/gbs.png" alt="" class="opimg deltb"> <img src="../../images/decorate/bj.png" alt="" class="opimg edittb"> </li>',
      ...
      //tab楼层编辑lidiv
    	...
    	$(demo).on('click','.edittb',function(e){
    		...
    		$('.tabBTModals1').fadeIn(200, function() {
    			...
    			$edittcThis = $(this);
    			var tab_ggwbm = editThis.parent(".kdli").find("a").children(".lcwx").attr("goodCodeInput");
    			$(this).children(".tabBtdiv1").find(".tab_ggwbm").val(tab_ggwbm);
    			var ewxpic = editThis.parent(".kdli").find("a").children(".lcwx").attr("src");
    			var exzpic = editThis.parent(".kdli").find("a").children(".lcxz").attr("src");
      ...
      //编辑
        $('.editTAbs').click(function(e){
    		...
    		editThis.parent(".kdli").find("a").children(".lcwx").attr("src",ewxpic);
    		editThis.parent(".kdli").find("a").children(".lcxz").attr("src",exzpic);
    		editThis.parent(".kdli").find("a").children(".lcwx").attr("goodcodeinputsign","goodcodeinputsign");
    		editThis.parent(".kdli").find("a").children(".lcwx").attr("goodCodeInput",tab_ggwbm);
    		editThis.parent(".kdli").find("a").children(".lcxz").attr("goodcodeinputsign","goodcodeinputsign");
    		editThis.parent(".kdli").find("a").children(".lcxz").attr("goodCodeInput",tab_ggwbm);
            closeC('.tabBTModals1'); 
        }); 
   ...
   $('.modal,.lbmodal').on("change",".file",function(e){
				 ...
				 for(var i= 1 ;i<imgNum;i++){
					...
					var	 edithyImg2 = p.find('.whyview .hyNav-box li').eq(i-1).children("a").children("div:last-child").children("img").attr("src");
					   ...
					   editqyxq="<tr class='qyqq whytr'>"
                                 ...
                                 +"		<dd>"
                           if(goldenFish == '1'){
                               editqyxq+=" 		<div class='gglogin"+i+"'><select><option value='1' selected>是</option><option value='0'>否</option></select></div>";
                           }else if(goldenFish === '0'){
                               editqyxq+=" 		<div class='gglogin"+i+"'><select><option value='1' >是</option><option value='0' selected>否</option></select></div>";
                           }else {
                               editqyxq+=" 		<div class='gglogin"+i+"'><select><option value='1' selected>是</option><option value='0'>否</option></select></div>";
                           }
	...
	$('.whyedit-layer').on('click','.savewhy',function(e){
		...
		for(var i= 0 ;i<imgNum;i++){ 
			...
			//锦鲤
			var goldenFish  =  $(this).parents('.whyedit-layer').children(".whymain-content").find("table tbody").children("tr").eq(i).find(".hytddiv:nth-child(2)").find("select").val();
			...
			//获取业务图片地址
			var tpdz1 = $(this).parents('.whyedit-layer').children(".whymain-content").find("table tbody").children("tr").eq(i).find(".hytddiv:nth-child(3)").find(".wxzpic .imgPerview").attr("src");
			var tpdz2 = $(this).parents('.whyedit-layer').children(".whymain-content").find("table tbody").children("tr").eq(i).find(".hytddiv:nth-child(4)").find(".xzpic .imgPerview").attr("src");
			...
			if(whymenuad!=null&&whymenuad!=''&&whymenuad!=undefined){
			    if(whymenuad.indexOf("syhhkj")>0){
                    editHxcd += "\<li class='whyli' "+newdbdh_ggwbmStr+"><a onclick="+"NewBottomJumpURL(\'"+whymenuad+"\',\'"+goldenFish+"\') goldenFish='"+goldenFish+"' turnHref='"+whymenuad+"' titleorname='"+titleorname+"' url='"+whymenuad+"'  turnJumpImage='"+tpdz2+"'>"
                }else if(whymenuad.indexOf("rr")>0){
					editHxcd += "\<li class='whyli' "+newdbdh_ggwbmStr+"><a onclick="+"addToProductStore.executeFvsStatcAdid(\'"+whymenuad+"&turnJumpImage="+tpdz2+"\',\'1\',this,\'02\',\'1\',null,\'"+goldenFish+"\') goldenFish='"+goldenFish+"' turnHref='"+whymenuad+"' titleorname='"+titleorname+"' bindturnhrefimage='bindturnhrefimage' url='"+whymenuad+"'>"
				}else if(whymenuad.indexOf("adid")>0){
					editHxcd += "\<li class='whyli' "+newdbdh_ggwbmStr+"><a onclick="+"addToProductStore.executeFvsStatcAdid(\'"+whymenuad+"&turnJumpImage="+tpdz2+"\',\'1\',this,\'02\',\'1\',null,\'"+goldenFish+"\') goldenFish='"+goldenFish+"' turnHref='"+whymenuad+"' titleorname='"+titleorname+"' url='"+whymenuad+"'>"
				}else{
					editHxcd += "\<li class='whyli' "+newdbdh_ggwbmStr+"><a onclick="+"NewBottomJumpURL(\'"+whymenuad+"\',\'"+goldenFish+"\') goldenFish='"+goldenFish+"' turnHref='"+whymenuad+"' titleorname='"+titleorname+"' url='"+whymenuad+"'  turnJumpImage='"+tpdz2+"'>"
				}
			}else{
        ...
        for(var i= 0 ;i<imgNum;i++){
            ...
            if(bgbmo == "none" || bgbmo == "" || bgbmo == undefined){
                ...
                bmgneditHtml+=`<img src="images/images.png" alt="" class="imagesZw" style='display:none;'>
                            ...
                            <p class="opacityP j-deleteImg" id="bmmkimg`+imgNum+`opacityP" onclick="deleteImg('bmmkimg`+imgNum+`')">删除</p>
...
$('.feedit-layer').on('click','.savebmgn',function(e){
    ...
    for(var i= 0 ;i<imgNum;i++){
        ...
        editBmgn += `<div class="inclass my_bjgn"><input type="hidden" class="hidetit" value="`+ seltit +`"><p class="md_des">`+ tit +`</p>`;
        if(seltit == 'bmtyll'){
        ...
        if(url != "none" && url!=""){
            ...
            editBmgn +=`<div class="bmzwbtn bmbtna"><a href="javascript:void(0);" oriData=""></a></div>`;
        }
        editBmgn +=`</div></li>`;
    }
