...
$.fn.imageMaps = function(setting){
	...
	function initAddClickEvent(){
		...
		$("#add_hot_area").unbind("click").click(function () {
			...
			$('.hotImgNumList').append(`<div class="hotImgItem">
				<div class="flex alignCenter inputDivPub">
					<div><span style="color:#FF0000;">*</span><span> 热区1链接：</span></div>
					<div class="flex alignCenter flex1">
						<input type="text" class="myinput" maxlength="100" maxlength="100" placeholder="请输入链接地址或选择">
						<button class="chooseBtnPub" type="button">选择聚合页/广告</button>
					</div>
				</div>
				<div class="flex alignCenter inputDivPub">
					<div><span> 点位名称：</span></div>
					<div class="flex alignCenter flex1"><input type="text" class="myinput" value="" placeholder="请输入点位名称"></div>
				</div>
				<div class="flex alignCenter inputDivPub mb0">
					<div><span> 点位编码：</span></div>
					<div class="flex alignCenter flex1"><input type="text" class="myinput" value="" placeholder="请输入点位编码"></div>
				</div>
				<div class="delBtnHot">删除</div>
			</div>`)
			bindMapEvent();
			updateAreaCount();
	...
	$(document).off('click',".delBtnHot").on('click','.delBtnHot',function() { //右侧删除
		let index = $('.hotImgNumList .hotImgItem').index($(this).parents(".hotImgItem")) //当前是删除的第几个
		$('.hot_area .delete').eq(index).click()
	})
	function initDeleteClickEvent(){
		...
		$('.hot_area .delete').unbind('click').click(function () {
			console.log('9090')
			// if($('.hotImgNumList .hotImgItem').length==1){
			// 	alert('至少保留一个热区')
			// 	return
			// }
			...
			$('.hotImgNumList .hotImgItem').eq(parseInt($(this).prev('.link_number_text').text())-1).remove()
	...
	this.each(function() {
		...
		if (hotImage != null) {
			...
			imageContainer.find('.position_container').css({
				...
				width : "calc(100% - 1px)",// 这里按照1.0，将hotImage改为imageMap
				height : "calc(100% - 2px)",
...
console.log(areaInfo);
			...
			if (areaInfoJson != null && areaInfoJson.length > 0) {
				...
				for (var j = 0; j < areaInfoJson.length; j++) {
					...
					var areaCode = areaInfoJson[j].areaCode||"";
					var areaMapInfo = areaInfoJson[j].areaMapInfo;
					$('.hotImgNumList').append(`
						<div class="hotImgItem">
							<div class="flex alignCenter inputDivPub">
								<div><span style="color:#FF0000;">*</span><span> 热区${j+1}链接：</span></div>
								<div class="flex alignCenter flex1">
									<input type="text" class="myinput" value="${areaLink}" maxlength="100" maxlength="100" placeholder="请输入链接地址或选择">
									<button class="chooseBtnPub" type="button">选择聚合页/广告</button>
								</div>
							</div>
							<div class="flex alignCenter inputDivPub">
								<div><span> 点位名称：</span></div>
								<div class="flex alignCenter flex1"><input type="text" class="myinput" value="${areaTitle}" placeholder="请输入点位名称"></div>
							</div>
							<div class="flex alignCenter inputDivPub mb0">
								<div><span> 点位编码：</span></div>
								<div class="flex alignCenter flex1"><input type="text" class="myinput" value="${areaCode}" placeholder="请输入点位编码"></div>
							</div>
							<div class="delBtnHot">删除</div>
						</div>
					`)
		...
		$('.position_container .map_position .map_position_bg').each(function(){
			...
			$(document).mousemove(function(event) {
				if (!mapPositionBg.data('mousedown')) return false;
					...
					// 拖动
					// console.log(new Array(Math.round(left),Math.round(top),Math.round(right),Math.round(bottom)).join(','),"拖动")
	...
	$('.position_container .map_position .resize').each(function(){
		...
		$(document).mousemove(function(event){
			...
			right = left + mapPosition.width();
			// resize
			// console.log(new Array(Math.round(left),Math.round(top),Math.round(right),Math.round(bottom)).join(','),"resize")
			$(setting.tag + '[name="areaItem"][ref='+ mapPosition.attr('ref') +'] .areaMapInfo').val(new Array(Math.round(left),Math.round(top),Math.round(right),Math.round(bottom)).join(','));
			return false;
		}).mouseup(function(event){
			mapPositionResize.data('mousedown', false);
...
function updateAreaCount() {
	...
	if (rowCount != null) {
		...
		$('.hotImgItem').each(function(i){
			$(this).find('div:first-child div:first-child span:last-child').html(`热区${i+1}链接：`)
		})
	}
}
};
...
function hotImgSub(){
	if($('.hotImgItem').length>0){
		let flag = true
		$('.hotImgItem').each(function(i) {
			if(!$(this).find('.inputDivPub:first-child input').val()){
				flag = false
				alert(`请输入/选择热区${i+1}链接`)
				return false
			}
		})
		if(!flag){
	...
	$('.hotImgItem').each(function(i) {
		arr.push({
			areaMapInfo: $('#areaItems tr').eq(i).find('input').val(),
			areaLink: $(this).find('.inputDivPub:first-child input').val(),
			areaTitle: $(this).find('.inputDivPub:nth-child(2) input').val(),
			areaCode: $(this).find('.inputDivPub:nth-child(3) input').val(),
		})
	})
	
	$('.modalsHot').fadeOut(200,function() {
		if(arr.length>0){
			$('#hotZoneImagesTable tr').eq(parseInt($('.modalsHot').attr("trData"))).find('.uploadImgBox').attr("json",JSON.stringify(arr)).parent().next().text(arr.length)
			console.log(arr)
		}else{
			$('#hotZoneImagesTable tr').eq(parseInt($('.modalsHot').attr("trData"))).find('.uploadImgBox').attr("json","").parent().next().text("0")
			console.log('没数据')
		}
	})
