	...
	$(function(){
	  ...
	  function qhli(){
		...
		*/
		...
		function restoreData(){
			if (supportstorage()) {
				htmlData = JSON.parse(localStorage.getItem("htmlData"));
				 //这里处理楼层懒加载 tab楼层异步点击的问题。
				if(!htmlData){
					setDatasrc();//设置data-src
		...
		function reDatasrc(){
			/**
				 * 20211215add 渲染页面获取img标签是否有data-src属性
			 ...
			 Array.from(imgs).forEach(function(el){
				 console.log(!eachTab(),"没有data-src属性");
				 if(!el.getAttribute("data-src") && !eachTab()){//如果没有data-src属性正常渲染
					 console.log("没src");
				 }else{
					 //有data-src就将data-src属性赋值给src
		...
		/**
		 * 2021add 1223 qsf 新的模板首次点击保存设置data-src，
		...
		function setDatasrc(){ 
			...
			var imgs = div.querySelectorAll("img");
			Array.from(imgs).forEach(function(el){
				if(!el.getAttribute("data-src")){//如果没有data-src属性正常渲染
					console.log("保存没src");
