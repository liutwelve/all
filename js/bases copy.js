	  ...
	  function qhli(){
		...
		*/
		...
		function restoreData(){
			...
			if (supportstorage()) {
				...
				if(htmlData && htmlData.step.length>0){
			//  console.log(i<=htmlData.step.length);
					for(var i=0 ; i<=htmlData.step.length ; i++){
						if(htmlData.step[i] && htmlData.step[i].indexOf("<img")>-1){
							// console.log(document.getElementsByClassName(htmlData.step[i]));
							var temp = htmlData.step[i].replace(/&/g,"&amp;");
							console.log(temp);
							 
							console.log(temp);
							// $(htmlData.step[i]).find("img");
							// $("#imgDiv").find(img).each(function(){ 
							// 	var imgId = $(this).attr("src");
							// 	var newImgUrl = getContextPath()+"/api/file/getFileById?moduleCode=1&attId="+imgId;
							// 	$(this).attr("src",newImgUrl) ; 
							// 	//alert($(this).attr("src",newImgUrl);
							// 	 //alert($(this).attr("src",newImgUrl);
							// 	 //alert($(this).attr(“src”));
							// 	}); 
							// console.log(,"二二");
		...
		*/
		...
		var $lcThis,
			$lctcThis,//添加弹窗元素 
			$edittcThis,//编辑this 
			tabLCTemplate = '<li class="swiper-slide kdli"> <a href="#{href}">#{label}</a> <img src="images/gbs.png" alt="" class="opimg deltb"> <img src="images/bj.png" alt="" class="opimg edittb"> </li>',
