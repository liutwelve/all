...
var iopBlockObj;//iop组件广告选择按钮
var iopTypeName="";//iop组件名称，用于生成随机的iop广告位编码
var swiperScale = null
...
//               $(".tabslc").tabs();
        ...
        updateContent:function (sign) {// 提交方法sign 0 直接提交 1 为鼠标移出提交
            $("#save").css('pointer-events','none');//锁定保存按钮，防止重复点击
            addAttrToTaga();//插码标签
            setGoodsCode();//获取插码值
            /**
...
            formData.append('content', content);
            ...
            formData.append("goodsMap",mapToString(goodsCodeMap));
            let config = {
              headers: {
...
              if(response.data.flag){
                    if('0' == sign){//保存成功
                        $("#save").css('pointer-events','');//移除防重复点击
       ...
       // 权益详情选择聚合 、广告
        ...
        .on('click','.edit',function(e) {
            ...
            $('.modals').fadeIn(200, function() {
                ...
                layer.find('.tab-content').find('.tc').each(function (i,v) {if(i==0){$(v).addClass('active')}if(i==1){$(v).removeClass('active')}});
                layer.css({
                    'margin-top':-(layer.height())/2,
                    'margin-left':-(layer.width())/2
                }).fadeIn(100);
                ...
                if(type=="head"){//标签大类
                    ...
                    for(var i= 1 ;i<imgNum;i++){
                        var editPicImgSrc = p.find('.myview .goodsImage img').eq(i-1).prop('src');
                            ...
                            if(!cmGoodCode){cmGoodCode = "IMAGE"+(new Date().getTime())}//赋值随机插码数据
                            goodcodeinputname = p.find('.myview .goodsImage a').eq(i-1).attr('goodcodeinputname') == undefined?"":p.find('.myview .goodsImage a').eq(i-1).attr('goodcodeinputname');
                        ...
                        editHtml += "<tr><th>点位名称：</th><td><input id='goodcodeinputname' maxlength='20' type='text' value='" + goodcodeinputname + "' style='float:left;margin-bottom: 0.05rem;'></td></tr>";
  ...
  case '导购':
	  ...
	  break;
                  ...
                  if(editPicBqImgSrc){
                    ...
                    var goodcodeinputname =  p.find('.view img').attr('goodcodeinputname') == undefined?"":p.find('.view img').attr('goodcodeinputname');
                    if(!goodcodeinput){goodcodeinput = "BUSIS"+(new Date().getTime())}//插码固定值
                    var editHtmlTitleSrc = p.find('.topbg').eq(0).prop('src'),
                        editHtmlTitle = "<tr class='jtggwtr'><th>广告位编码：</th><td><input id='goodCodeInput' type='text' value='"+goodcodeinput+"' goodcodeinputsign='goodcodeinputsign' class='goodcodeinputsign' style='float:left;'></td></tr>" +
                            "<tr class='jtggwtr'><th>点位名称：</th><td><input id='goodcodeinputname' maxlength='20' type='text' value='"+goodcodeinputname+"' style='float:left;'></td></tr>" +
                            "<tr class='jtggwtr'><th>图片：</th><td><div class='selectImg'><p class='title_url'>" + editHtmlTitleSrc + "</p><input type='file' value='11' class='file'></div><p class='yulanImg'><img src='" + editHtmlTitleSrc + "' alt=''/></p></td></tr>";
                    $('#htmlCon table').html('').prepend(editHtmlTitle);
                    $('.selectP').hide();
                    $('.selectP1').hide();
                    ...
                    var str = `<tbody class="zpcjEditBox">
						...
						</div><div class="shownamebox j_name" style="display: block;"><p class="name" id="headerImg">`+headImg+`</p><p class="del j_del">删除</p></div><p class="yulanImg" style="display: block;"><img class="bgimg1" src="`+headImg+`" alt="">
						</p></div></div></td></tr>
                    ...
                    $('.selectP1').hide();
                    $('#htmlCon').show();
                    var $this = p.find(".sellerInfo");
                    var bgImg = $($this).css("background-image");
                    var reg = new RegExp('"','g');
                    bgImg = bgImg == undefined||bgImg == "none"?'':bgImg.split("(")[1].split(")")[0].replace(reg,'');
                    var headImg = $($this).find('.infobox').find('.left').find('.tx').find('img').attr('src');//头像
                    var post = $($this).find('.infobox').find('.left').find('.myinfo').find('.post').html();//职位
                    var jobid = $($this).find('.infobox').find('.left').find('.myinfo').find('.jobid').html();//工号
                    var groupname = $($this).find('.infobox').find('.left').find('.myinfo').find('.groupname').html();//组织名称
                    var scorebox = $($this).find('.infobox').find('.right').find('.scorebox').html();//积分栏
                    let codebox = $($this).find('.infobox').find('.right').find('.codebox').html();//右上角分享二维码
                    var scoregoodsinput = "";
                    var scoregoodsinputname = "";
                    if(scorebox){
                        scoregoodsinput = $($this).find('.infobox').find('.right').find('.scorebox').attr("goodcodeinput");//积分插码值
                        scoregoodsinputname = $($this).find('.infobox').find('.right').find('.scorebox').attr("goodcodeinputname");//积分插码值
                        if(!scoregoodsinput){scoregoodsinput = "SCOREBOX"+(new Date().getTime());}
                        if(!scoregoodsinputname){scoregoodsinputname = "";}
                    }
                    console.log($this);
                    var str = `
					<tbody class="zpcjEditBox sellerInfoCss" id="resetCss"><tr><td><div class="flexbox mt15"><div class="w10"><p class="cred">*</p><p>背景图：</p></div>
					<div class="flex1 uploadImgBox"><div class="flexbox"><div class="selectImg"><button class="uploadBtn">浏览</button><input type="hidden"/><input type="file" value="" class="file myinput"></div>
					<p class="tishiTxt ml5">建议上传图片尺寸750*502</p></div><div class="shownamebox j_name" style="display: block;"><p class="name" id="bgImg">`+bgImg+`</p><p class="del j_del">删除</p></div>
					<p class="yulanImg" style="display: block;"><img class="bgimg1" src="`+bgImg+`" alt=""></p>
					</div></div></td></tr><tr><td><div class="flexbox mt15"><div class="w10"><p class="cred">*</p><p>卖家信息：</p></div><div class="flex1"><div class='ljcd' style="margin-left:0">`
                    if(headImg){
                        str+=`<label class="flexbox align-c"><input type='checkbox' name='checkInfo' id="headImg" class='nninput' checked/>头像</label>`;
                    }else{
                        str+=`<label class="flexbox align-c"><input type='checkbox' name='checkInfo' id="headImg" class='nninput'/>头像</label>`;
                    }
                    str+=`<label class="flexbox align-c ml5"><input type='checkbox' name='checkInfo' class='nninput' checked disabled/>卖家昵称</label>`
                    if(post){
                        str+=`<label class="flexbox align-c ml5"><input type='checkbox' name='checkInfo' id="post" class='nninput' checked/>职位</label>`;
                    }else{
                        str+=`<label class="flexbox align-c ml5"><input type='checkbox' name='checkInfo' id="post" class='nninput'/>职位</label>`;
                    }
                    if(jobid){
                        str+=`<label class="flexbox align-c ml5"><input type='checkbox' name='checkInfo' id="jobid" class='nninput' checked/>工号</label>`;
                    }else{
                        str+=`<label class="flexbox align-c ml5"><input type='checkbox' name='checkInfo' id="jobid" class='nninput'/>工号</label>`;
                    }
                    if(groupname){
                        str+=`<label class="flexbox align-c ml5"><input type='checkbox' name='checkInfo' id="groupname" class='nninput' checked/>组织名称</label>`;
                    }else{
                        str+=`<label class="flexbox align-c ml5"><input type='checkbox' name='checkInfo' id="groupname" class='nninput' />组织名称</label>`;
                    }
                    if(scorebox){
                        str+=`<label class="flexbox align-c ml5"><input type='checkbox' id="scorebox" name='checkInfo' class='nninput' checked/>积分</label>`;
                    }else{
                        str+=`<label class="flexbox align-c ml5"><input type='checkbox' id="scorebox" name='checkInfo' class='nninput'/>积分</label>`;
                    }
                    if(codebox){
                        str+=`<label class="flexbox align-c ml5"><input type='checkbox' id="codebox" name='checkInfo' class='nninput' checked/>二维码</label>`;
                    }else{
                        str+=`<label class="flexbox align-c ml5"><input type='checkbox' id="codebox" name='checkInfo' class='nninput'/>二维码</label>`;
                    }
                    str+=`</div><p class="tishiTxt">选中后显示，未选中则不显示</p></div></div></td></tr><tr><td><div class="flexbox align-s mt15"><div class="w10"><p class="cred">*</p><p>默认头像：</p></div>
					<div class="flex1 uploadImgBox"><div class="flexbox"><div class="selectImg"><button class="uploadBtn">浏览</button><input type="hidden"/><input type="file" value="" class="file myinput"></div><p class="tishiTxt ml5">建议上传图片尺寸80*80</p></div>`
                    if(headImg){
                        str+=`<div class="shownamebox j_name" style="display: block;"><p class="name" id="headerImg">`+headImg+`</p><p class="del j_del">删除</p></div><p class="yulanImg" style="display: block;"><img class="bgimg1" src="`+headImg+`" alt=""></p>`;
                    }else {
                        str+=`<div class="shownamebox j_name"><p class="name" id="headerImg"></p><p class="del j_del">删除</p></div><p class="yulanImg"><img class="bgimg1" src="" alt=""></p>`;
                    }
                    str+=`</div></div></td></tr><tr><td>`;
                    if(scoregoodsinput){
                        str+=`<div class="flexbox align-c mt15"><div class="w10"><p>积分广告位编码：</p></div><div class="flex1"><input type="text" id="scoregoodsinput" maxlength="48" class="myinput" value="`+scoregoodsinput+`" placeholder="请输入广告位编码"></div></div>`;
                        str+=`<div class="flexbox align-c mt15"><div class="w10"><p>积分点位名称：</p></div><div class="flex1"><input type="text" id="scoregoodsinputname" maxlength="20" class="myinput" value="`+scoregoodsinputname+`" placeholder="请输入点位名称"></div></div>`;
                    }else {
                        str+=`<div class="flexbox align-c mt15"><div class="w10"><p>积分广告位编码：</p></div><div class="flex1"><input type="text" id="scoregoodsinput" maxlength="48" class="myinput" value="" placeholder="请输入广告位编码"></div></div>`;
                        str+=`<div class="flexbox align-c mt15"><div class="w10"><p>积分点位名称：</p></div><div class="flex1"><input type="text" id="scoregoodsinputname" maxlength="20" class="myinput" value="" placeholder="请输入点位名称"></div></div>`;
                    }
                    str+=`</td></tr><tr><td><div class="flexbox align-c mt15"><div class="w10"><p></p></div>
					<div class="flex1 flexbox align-c"><div class="addbtn j_addbtn"><img class="icon" src="../../images/decorate/sellerInfo/cpn_add.png" alt=""><span class="txt">添加操作项</span></div></div>
					</div></td></tr></tbody>`;
                    $('#htmlCon table').css({'margin-top':'0'}).html('').append(str);
                    var optsStr = "";//操作项配置html
                    //操作项HTML回显
                    var optNum = 0;
                    $this.find(".myIconlist").find('ul').find("li").each(function (i,v) {
                        let imgSrc = $(v).find("img").attr("src");//图片地址
                        let optName = $(v).find("div").html();//操作名称
                        let optType = $(v).attr("opttype");//操作类型
                        let jumpurl = $(v).attr("jumpurl");//跳转地址
                        let goodsInputCode = $(v).attr("goodcodeinput");//广告位编码
                        let goodsInputCodeName = $(v).attr("goodcodeinputname");//广告位编码
                        let positionname = $(v).attr("positionname");//单选框按钮名称
                        if(!positionname){
                            return true;
                        }
                        optNum ++;
                        if(!goodsInputCode){goodsInputCode="";}
                        if(!goodsInputCodeName){goodsInputCodeName="";}
                        optsStr +=`<tr class="j_optionFlag"><td><div class="flexbox align-s mt15"><div class="w10"><p class="j_optionSort">操作项`+optNum+`：</p></div><div class="flex1 flexbox align-s"><div class="bgbox">`+
                        `<div class="flexbox align-c"><div class="w10"><p>广告位编码：</p></div><div class="flex1"><input type="text" class="myinput" name="goodscode" value="`+goodsInputCode+`" maxlength="48" placeholder="请输入广告位编码"></div></div>
                        <div class="flexbox align-c mt15"><div class="w10"><p>点位名称：</p></div><div class="flex1"><input type="text" class="myinput" name="goodscodename" value="`+goodsInputCodeName+`" maxlength="20" placeholder="请输入点位名称"></div></div>
                        <div class="flexbox align-s mt15"><div class="w10"><p class="cred">*</p><p>图标：</p></div><div class="flex1 uploadImgBox"><div class="flexbox"><div class="selectImg"><button class="uploadBtn">浏览</button>
                        <input type="hidden"/><input type="file" value="" class="file myinput"></div><p class="tishiTxt ml5">建议上传图片尺寸64*64</p></div><div class="shownamebox j_name" style="display: block;"><p class="name">`+imgSrc+`</p>
                        <p class="del j_del">删除</p></div><p class="yulanImg" style="display: block;"><img class="bgimg1" src="`+imgSrc+`" alt=""></p></div></div><div class="flexbox mt15"><div class="w10"><p class="cred">*</p>
                        <p>文案：</p></div><div class="flex1"><input type="text" class="myinput" name="btnname" value="`+optName+`" placeholder="请输入文案" maxlength="4"><p class="tishiTxt">最多可输入4个字符</p></div></div><div class="flexbox align-c">
                        <div class="w10"><p class="cred">*</p><p>执行：</p></div><div class="flex1"><select class="myinput" name="opttype" style="display:block;" onchange="sellerCardSecChange(this);">`;
                        if(optType == 'qywx'){
                            optsStr +=`<option value="qywx" selected>弹出企业微信弹窗</option><option value="callPhone">拨打系统电话</option><option value="qrcode">弹出页面二维码</option><option value="share">页面分享</option><option value="tourl">跳转链接</option>`;
                        }
                        if(optType == 'qrcode'){
                            optsStr +=`<option value="qywx">弹出企业微信弹窗</option><option value="callPhone">拨打系统电话</option><option value="qrcode" selected>弹出页面二维码</option><option value="share">页面分享</option><option value="tourl">跳转链接</option>`;
                        }
                        if(optType == 'callPhone'){
                            optsStr +=`<option value="qywx">弹出企业微信弹窗</option><option value="callPhone" selected>拨打系统电话</option><option value="qrcode">弹出页面二维码</option><option value="share">页面分享</option><option value="tourl">跳转链接</option>`;
                        }
                        if(optType == 'share'){
                            optsStr +=`<option value="qywx">弹出企业微信弹窗</option><option value="callPhone">拨打系统电话</option><option value="qrcode">弹出页面二维码</option><option value="share" selected>页面分享</option><option value="tourl">跳转链接</option>`;
                        }
                        if(optType == 'tourl'){
                            optsStr +=`<option value="qywx">弹出企业微信弹窗</option><option value="callPhone">拨打系统电话</option><option value="qrcode">弹出页面二维码</option><option value="share">页面分享</option><option value="tourl" selected>跳转链接</option>`;
                        }
                        if(!optType){
                            optsStr +=`<option value="qywx">弹出企业微信弹窗</option><option value="callPhone">拨打系统电话</option><option value="qrcode">弹出页面二维码</option><option value="share">页面分享</option><option value="tourl">跳转链接</option>`;
                        }
                        optsStr +=`</select></div></div>`;
                        if(optType == 'tourl'){
                            optsStr +=`<div class="flexbox align-c mt15"><div class="w10"><p class="cred">*</p><p>链接：</p></div><div class="flex1"><input type="text" class="myinput" name="jumpurl"
                            value="`+jumpurl+`" placeholder="请输入跳转链接"  maxlength="100"></div></div>`;
                        }else {
                            optsStr +=`<div class="flexbox align-c mt15" style="display: none;"><div class="w10"><p class="cred">*</p><p>链接：</p></div><div class="flex1"><input type="text" class="myinput" name="jumpurl"
                            value="" placeholder="请输入跳转链接"  maxlength="2000"></div></div>`;
                        }
                        optsStr += `</div><div class="delbtn j_delbtn"><img class="icon" src="../../images/decorate/sellerInfo/cpn_minus.png" alt=""><span class="txt">删除</span></div></div></div></td></tr>`;
                    });
                    $(document).find(".j_addbtn").parents("tr").before(optsStr);
                    $(document).find(".j_addbtn").on("click",function(){
                        var datastr = "OPTITEM" + new Date().getTime()+optNum;//广告位编码默认值
                        optNum ++;
                        let str = `
						<tr class="j_optionFlag">
							<td>
								<div class="flexbox align-s mt15"><div class="w10"><p class="j_optionSort">操作项`+optNum+`：</p></div>
									<div class="flex1 flexbox align-s">
										<div class="bgbox">
											<div class="flexbox align-c"><div class="w10"><p>广告位编码：</p></div>
												<div class="flex1"><input type="text" class="myinput" name="goodscode" value="`+datastr+`" maxlength="48" placeholder="请输入广告位编码"></div>
											</div>
											<div class="flexbox align-c mt15"><div class="w10"><p>点位名称：</p></div>
												<div class="flex1"><input type="text" class="myinput" name="goodscodename" value="" maxlength="20" placeholder="请输入点位名称"></div>
											</div>
											<div class="flexbox align-s mt15"><div class="w10"><p class="cred">*</p><p>图标：</p></div>
												<div class="flex1 uploadImgBox">
													<div class="flexbox"><div class="selectImg"><button class="uploadBtn">浏览</button><input type="hidden"/>
													<input type="file" value="" class="file myinput"></div><p class="tishiTxt ml5">建议上传图片尺寸64*64</p></div>
													<div class="shownamebox j_name"><p class="name"></p><p class="del j_del">删除</p></div>
													<p class="yulanImg"><img class="bgimg1" src="" alt=""></p>
												</div>
											</div>
											<div class="flexbox mt15"><div class="w10"><p class="cred">*</p><p>文案：</p></div>
												<div class="flex1"><input type="text" class="myinput" name="btnname" value="" placeholder="请输入文案" maxlength="4"><p class="tishiTxt">最多可输入4个字符</p></div>
											</div>
											<div class="flexbox align-c"><div class="w10"><p class="cred">*</p><p>执行：</p></div>
												<div class="flex1"><select class="myinput" name="opttype" style="display:block;" onchange="sellerCardSecChange(this);">
												<option value="qywx">弹出企业微信弹窗</option><option value="callPhone">拨打系统电话</option><option value="qrcode">弹出页面二维码</option><option value="share">页面分享</option><option value="tourl">跳转链接</option>
												</select></div>
											</div>
											<div class="flexbox align-c mt15" style="display: none;"><div class="w10"><p class="cred">*</p><p>链接：</p></div>
											    <div class="flex1"><input type="text" class="myinput" name="jumpurl" value=""placeholder="请输入跳转链接" maxlength="2000"></div>
                                            </div>
										</div>
										<div class="delbtn j_delbtn"><img class="icon" src="../../images/decorate/sellerInfo/cpn_minus.png" alt=""><span class="txt">删除</span></div>
									</div>
								</div>
							</td>
						</tr>`
                        $(this).parents("tr").before(str);
                        if($(".sellerInfoCss").find(".bgbox").length>=16){
                    ...
                    function sort(){
                        $(".j_optionFlag").each(function(index,item){
                            $(this).find(".j_optionSort").text("操作项"+(index+1)+"：");
                        })
                    }
                    ...
                    $('#htmlCon').show();
                    var editPicImgSrc = p.find('.cjview img.testimg').prop('src');
                    var adId = p.find('.shareImgLeftYa').attr('adId');
                    var adTitle = p.find('.shareImgLeftYa').attr('adTitle');
                    if(!adId){adId="";}if(!adTitle){adTitle="";}
                    var goodcodeinputvideo = p.find('.saLeftImg').attr('goodcodeinput');//视频封面图插码值
                    if(!goodcodeinputvideo){goodcodeinputvideo="SHAREAD" + new Date().getTime();}
                    var goodcodeinputnamevideo = p.find('.saLeftImg').find('img').attr('goodcodeinputname');//视频封面图点位名称
                    if(!goodcodeinputnamevideo){goodcodeinputnamevideo="";}
                    var goodcodeinputdetail = p.find('.saTopImg').attr('goodcodeinput');//广告详情图插码值
                    if(!goodcodeinputdetail){goodcodeinputdetail="SHAREAD" + new Date().getTime()+"1";}
                    var goodcodeinputnamedetail = p.find('.saTopImg').find('img').attr('goodcodeinputname');//广告详情图点位名称
                    if(!goodcodeinputnamedetail){goodcodeinputnamedetail="";}
                    var goodcodeinputshare = p.find('.shareImgLeftYa').attr('goodcodeinput');//广告分享赚插码值
                    if(!goodcodeinputshare){goodcodeinputshare="SHAREAD" + new Date().getTime()+"2";}
                    var goodcodeinputnameshare = p.find('.shareImgLeftYa').attr('goodcodeinputname');//广告分享赚点位名称
                    if(!goodcodeinputnameshare){goodcodeinputnameshare="";}
                    var goodcodeinputbuy = p.find('.shareImgRightYa').attr('goodcodeinput');//广告购买省插码值
                    if(!goodcodeinputbuy){goodcodeinputbuy="SHAREAD" + new Date().getTime()+"3";}
                    var goodcodeinputnamebuy = p.find('.shareImgRightYa').attr('goodcodeinputname');//广告购买省点位名称
                    if(!goodcodeinputnamebuy){goodcodeinputnamebuy="";}
                    ...
                    var str = `<tbody class="zpcjEditBox">
                         <tr>
                            <td>
                                <div class="flexbox align-c mt15">
                                    <div class="w10"><p class="cred">*</p><p>广告：</p></div>
                                    <div class="flex1 flexbox align-c">
                                        <input type="text" class="myinput" readonly value="`+adTitle+`" placeholder="请选择广告" id="activeTitle" maxlength="100">
                                        <input type="hidden" class="shareAd" id="shareAd" value="`+adId+`">
                                        <div class="popChooseBtnYa shareAdBlock">选择广告</div>
                                    </div>
                                </div>
                            </td>
                        </tr>
						<tr>
							<td>
								<div class="flexbox align-s mt15">
									<div class="w10"><p class="cred">*</p><p>广告背景图：</p></div>
									<div class="flex1 uploadImgBox">
										<div class="selectImg">
											<button class="uploadBtn">浏览</button><span class="uploadImgTag">建议上传图片尺寸：690*296px，大小不超过100K</span><input type="hidden"/>
											<input type="file" value="" class="file myinput">
										</div>
										<div class="shownamebox shareBackCom j_name" style="display: block;">
											<p class="name" id="bkImg">${getImgUrl('saBackBlock')}</p>
						...
						<tr>
							<td>
								<div class="flexbox align-s mt15">
									<div class="w10"><p class="cred">*</p><p>广告视频封面图：</p></div>
									<div class="flex1 uploadImgBox">
										...
										<div class="shownamebox adVideoImgYa j_name" style="display: block;">
											<p class="name" id="adImg">广告视频封面图.png</p>
											<p class="del j_del">删除</p>
										</div>
						...
						<tr>
                            <td>
                                <div class="flexbox align-c mt15">
                                    <div class="w10"><p class="cred">*</p><p>视频图广告位编码：</p></div>
                                    <div class="flex1 flexbox align-c">
                                        <input type="text" class="myinput" value="`+goodcodeinputvideo+`" placeholder="请输入广告位编码" id="shareAdVideoCode" maxlength="50">
                                    </div>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <div class="flexbox align-c mt15">
                                    <div class="w10"><p class="cred">*</p><p>视频图点位名称：</p></div>
                                    <div class="flex1 flexbox align-c">
                                        <input type="text" class="myinput" value="`+goodcodeinputnamevideo+`" placeholder="请输入点位名称" id="shareAdVideoCodeName" maxlength="20">
                                    </div>
                                </div>
                            </td>
                        </tr>
						...
						<tr>
                            <td>
                                <div class="flexbox align-c mt15">
                                    <div class="w10"><p class="cred">*</p><p>详情图广告位编码：</p></div>
                                    <div class="flex1 flexbox align-c">
                                        <input type="text" class="myinput" value="`+goodcodeinputdetail+`" placeholder="请输入广告位编码" id="shareAdDetailCode" maxlength="50">
                                    </div>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <div class="flexbox align-c mt15">
                                    <div class="w10"><p class="cred">*</p><p>详情图点位名称：</p></div>
                                    <div class="flex1 flexbox align-c">
                                        <input type="text" class="myinput" value="`+goodcodeinputnamedetail+`" placeholder="请输入点位名称" id="shareAdDetailCodeName" maxlength="20">
                                    </div>
                                </div>
                            </td>
                        </tr>
								...
								<div class="flexbox align-s mt15">
									...
									<div class="flex1 uploadImgBox">
										...
										<div class="shownamebox adShareBtnYa j_name" style="display: block;">
											<p class="name" id="shareImg">广告分享按钮图.png</p>
											<p class="del j_del">删除</p>
										</div>
										<p class="yulanImg" style="display:block"><img class="bgimg1" src="" alt=""></p>
									</div>
						...
						<tr>
                            <td>
                                <div class="flexbox align-c mt15">
                                    <div class="w10"><p class="cred">*</p><p>分享按钮图广告位编码：</p></div>
                                    <div class="flex1 flexbox align-c">
                                        <input type="text" class="myinput" value="`+goodcodeinputshare+`" placeholder="请输入广告位编码" id="shareAdShareCode" maxlength="50">
                                    </div>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <div class="flexbox align-c mt15">
                                    <div class="w10"><p class="cred">*</p><p>分享按钮图点位名称：</p></div>
                                    <div class="flex1 flexbox align-c">
                                        <input type="text" class="myinput" value="`+goodcodeinputnameshare+`" placeholder="请输入点位名称" id="shareAdShareCodeName" maxlength="20">
                                    </div>
                                </div>
                            </td>
                        </tr>
								...
								<div class="flexbox align-s mt15">
									...
									<div class="flex1 uploadImgBox">
										...
										<div class="shownamebox adSelfBtnYa j_name" style="display: block;">
											<p class="name" id="buyImg">广告自购按钮图.png</p>
											<p class="del j_del">删除</p>
										</div>
										<p class="yulanImg" style="display:block"><img class="bgimg1" src="" alt=""></p>
									</div>
						...
						<tr>
                            <td>
                                <div class="flexbox align-c mt15">
                                    <div class="w10"><p class="cred">*</p><p>自购按钮图广告位编码：</p></div>
                                    <div class="flex1 flexbox align-c">
                                        <input type="text" class="myinput" value="`+goodcodeinputbuy+`" placeholder="请输入广告位编码" id="shareAdBuyCode" maxlength="50">
                                    </div>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <div class="flexbox align-c mt15">
                                    <div class="w10"><p class="cred">*</p><p>自购按钮图点位名称：</p></div>
                                    <div class="flex1 flexbox align-c">
                                        <input type="text" class="myinput" value="`+goodcodeinputnamebuy+`" placeholder="请输入点位名称" id="shareAdBuyCodeName" maxlength="20">
                                    </div>
                                </div>
                            </td>
                        </tr>
                    ...
                    // 回填默认数据
                    $('.adVideoImgYa p.name').html(p.find('.backImgFlag').attr("src"))
                    $('.adShareBtnYa p.name').html(p.find('.shareImgLeftYa').attr("src"))
                    $('.adSelfBtnYa p.name').html(p.find('.shareImgRightYa').attr("src"))
                    $('.adDetailImgYa p.name').html(p.find('.saTopImg img').attr("src"))
                    $('.adVideoImgYa+.yulanImg img').attr("src",p.find('.backImgFlag').attr("src"))
                    $('.adShareBtnYa+.yulanImg img').attr("src",p.find('.shareImgLeftYa').attr("src"))
                    $('.adSelfBtnYa+.yulanImg img').attr("src",p.find('.shareImgRightYa').attr("src"))
                    $('.adDetailImgYa+.yulanImg img').attr("src",p.find('.saTopImg img').attr("src"))
                    //删除图片
                    ...
                    $('.selectP1').hide();
                    $('#htmlCon').show();
                    var adId = p.find('#dragDiv').attr('adId');
                    var goodcodeinput = p.find('#dragDiv').attr('goodcodeinput');
                    if(!goodcodeinput){goodcodeinput="ACTPOP" + new Date().getTime();}
                    var goodcodeinputname = p.find('#dragDiv').attr('goodcodeinputname');
                    if(!goodcodeinputname){goodcodeinputname="";}
                    var isClose = "1";
                    if(p.find(".bottomImgYa").hasClass("none")){
                        isClose = "0";
                    }
                    if(!adId){adId="";}
                    var editPicImgSrc = p.find('.cjview img.testimg').prop('src');
                    var str = `<tbody class="zpcjEditBox">
                         <tr>
                             <td>
                                 <div class="flexbox align-c mt15">
                                    <div class="w10"><p>广告位编码：</p></div>
                                        <div class="flex1 flexbox align-c">
                                            <input type="text" class="myinput" value="`+goodcodeinput+`" placeholder="请输入广告位编码" maxlength="50" id="activeGoodsCode">
                                        </div>
                                 </div>
                             </td>
                        </tr>
                        <tr>
                             <td>
                                 <div class="flexbox align-c mt15">
                                    <div class="w10"><p>点位名称：</p></div>
                                        <div class="flex1 flexbox align-c">
                                            <input type="text" class="myinput" value="`+goodcodeinputname+`" placeholder="请输入点位名称" maxlength="20" id="activeGoodsCodeName">
                                        </div>
                                 </div>
                             </td>
                        </tr>
						<tr>
							<td>
								<div class="flexbox align-s mt15">
									<div class="w10"><p class="cred">*</p><p>悬浮球图片：</p></div>
									<div class="flex1 uploadImgBox">
										<div class="selectImg upTipFlagYa">
											<div><button class="uploadBtn">浏览</button><span class="uploadImgTag">建议上传图片尺寸：120*120px，大小不超过100K</span></div>
											<input type="hidden"/><input type="file" value="" class="file myinput">
										</div>
										<div class="shownamebox floatBallImgM j_name" style="display:block"><p class="name" id="popImg"></p><p class="del j_del">删除</p></div>
										<p class="yulanImg" style="display:block"><img class="bgimg1" src="" alt=""></p>
									</div>
								</div>
							</td>
						...
						<tr>
							<td>
								<div class="flexbox align-c mt15">
									<div class="w10"><p class="cred">*</p><p>跳转链接：</p></div>
									<div class="flex1 flexbox align-c">
										<input type="text" class="myinput" value="`+adId+`" placeholder="请输入跳转链接" maxlength="100" id="activeTitle">
										<div class="popChooseBtnYa popAdBlock">选择广告/聚合页</div>
									</div>
								</div>
							</td>
						</tr>
                        ...
                        if(isClose == '0'){
                            str += `<label><input type='radio' name='closeFlag' value='1' class='nninput' />是</label><label style="margin-left:5px"><input type='radio' name='closeFlag' value='0' class='nninput' checked/>否</label>`;
                        }else{
                            str += `<label><input type='radio' name='closeFlag' value='1' class='nninput' checked/>是</label><label style="margin-left:5px"><input type='radio' name='closeFlag' value='0' class='nninput'/>否</label>`;
                        }
                    str += `</div>
									</div>
								</div>
						...
						<tr class="closeBlockYa">
							<td>
								<div class="flexbox align-s mt15">
									<div class="w10"><p class="cred">*</p><p>关闭图标：</p></div>
									<div class="flex1 uploadImgBox">
										<div class="selectImg upTipFlagYa">
											<div><button class="uploadBtn">浏览</button><span class="uploadImgTag">建议上传图片尺寸：30*30px，大小不超过100K</span></div>
											<input type="hidden"/><input type="file" value="" class="file myinput">
										</div>
										<div class="shownamebox closeImgM j_name" style="display:block"><p class="name" id="closeBtn"></p><p class="del j_del">删除</p></div>
										<p class="yulanImg" style="display:block"><img class="bgimg1" src="../../images/decorate/floatBlockCom/5.png" alt=""></p>
									</div>
								</div>
							</td>
						</tr>
                    ...
                    // 回填默认数据
                    $('.floatBallImgM p.name').html(p.find('.rightFloatBlock .topImgYa img').attr("src"))
                    $('.floatBallImgM+.yulanImg img').attr("src",p.find('.rightFloatBlock .topImgYa img').attr("src"))
                    $('.closeImgM p.name').html(p.find('.bottomImgYa img').attr("src"))
                    $('.closeImgM+.yulanImg img').attr("src",p.find('.bottomImgYa img').attr("src"))
                    //删除图片
                    $(".j_del").click(function(){
                        $(this).siblings(".name").text("");
                        $(this).parent(".j_name").hide();
                    ...
                    layer.find('.selectLi').find('li').each(function (i,v) {if(i==0){$(v).html('IOP推荐')}if(i==1){$(v).html('补位广告')}});
                    layer.find('.tab-content').find('.tc').each(function (i,v) {if(i==0){$(v).removeClass('active')}if(i==1){$(v).addClass('active')}});
                    $('.selectP').hide();
                    $('.selectP1').hide();
                    $('#htmlCon').show();
                    var roomId = p.find('.iopAdSwiperBox').attr('id');//元素id
                    var search = p.find(".iopAdSearch").get(0);
                    var tsMsg = "策划ID或者广告编码出现重复现象，具体行数为lineNums，请检查后重新填写！";//编码重复提示语
                    var editPicImgSrc = p.find('.cjview img.testimg').prop('src');
                    var searchBk = p.find('.iopAdSearch').find('div').css('background-color');
                    var searchBorder = p.find('.iopAdSearch').find('div').css('border-color');
                    var iopadgoodcode = p.find('.iopAdSearBtn').attr("goodcodeinput");
                    if(!iopadgoodcode){iopadgoodcode = "IOPSBTN" + new Date().getTime();}
                    var iopadgoodcodename = p.find('.iopAdSearBtn').attr("goodcodeinputname");
                    if(!iopadgoodcodename){iopadgoodcodename = ""}
                    showEditPop();
                    function showEditPop(){
                        $.ajax({
                            url:vm.path + '/tWlmDecorate/queryIopBlockData.do', type:'post', async:false,
                            data:{decorateId:GetQueryString('id'),roomId:roomId},
                            success:function (data) {
                                if(data.code=='suc'){
                                    var str = `<tbody class="zpcjEditBox">
							<tr>
								<td>
									<div class="flexbox align-s mt15">
										<div class="w10"><p class="cred">*</p><p>背景图：</p></div>
										<div class="flex1 uploadImgBox">
											<div class="flexbox">
												<div class="selectImg"><button class="uploadBtn">浏览</button><input type="hidden"/><input type="file" accept="image/png,image/jpeg" value="" class="file myinput"></div>
												<p class="tishiTxt ml5">建议上传图片尺寸750*390px</p>
											</div>
											<div class="shownamebox j_name" style="display: block;"><p class="name iopBackImgName1"></p><p class="del j_del">删除</p></div>
								...
								<td>
									<div class="flexbox align-c mt15">
										...
										<div class="flex1 flexbox align-c">
											<div class='ljcd' style="margin-left:0">`
                                    if(search){
                                        str += `<label><input type='radio' name='iopFlagYa' value='1' class='nninput' checked/>是</label>
                                    <label style="margin-left:5px"><input type='radio' name='iopFlagYa' value='0' class='nninput'/>否</label>`
                                    }else {
                                        str += `<label><input type='radio' name='iopFlagYa' value='1' class='nninput' />是</label>
                                    <label style="margin-left:5px"><input type='radio' name='iopFlagYa' value='0' class='nninput' checked/>否</label>`
                                    }
                                    str+=`</div>
										</div>
									</div>
							...
							<tr>
								<td>
									<div class="flexbox align-s mt15">
										<div class="w10"><p class="cred">*</p><p>查询按钮图：</p></div>
										<div class="flex1 uploadImgBox">
											<div class="flexbox">
												<div class="selectImg"><button class="uploadBtn">浏览</button><input type="hidden"/><input type="file" accept="image/png,image/jpeg" value="" class="file myinput"></div>
												<p class="tishiTxt ml5">建议上传图片尺寸132*60px</p>
											</div>
											<div class="shownamebox j_name" style="display: block;"><p class="name iopSearchBtnImgName1"></p><p class="del j_del">删除</p></div>
							...
							<tr>
								<td>
									<div class="flexbox align-c mt15">
										<div class="w10">查询按钮广告位编码：</div>
										<div class="flex1 flexbox align-c">
											<input type="text" value="`+iopadgoodcode+`" placeholder="请输入广告位编码" id="iopadgoodcode">
										</div>
									</div>
								</td>
							</tr>
							<tr>
								<td>
									<div class="flexbox align-c mt15">
										<div class="w10">查询按钮点位名称：</div>
										<div class="flex1 flexbox align-c">
											<input type="text" value="`+iopadgoodcodename+`" placeholder="请输入点位名称" maxlength='20' id="iopadgoodcodename">
										</div>
									</div>
								</td>
							</tr>
							<tr>
								<td>
									<div class="iopSearchTablePart">
										<span>搜索</span>
										<div><span>广告位编码：</span><input type="text" placeholder="请输入广告位编码" id="adCodeIopYa"></div>
										<div><span>策划编码：</span><input type="text" placeholder="请输入策划编码" id="chCodeIopYa"></div>
										<button type="button" style="border-radius:10px; margin-left:10px; margin-right:10px; vertical-align: unset;" class="query">查询</button>
										<button type="button" style="border-radius:10px; vertical-align: unset;" class="reset">重置</button>
									</div>
									<table id="tableByIdYa">
										<thead>
											<tr><th style="width:10px"><input type="checkbox" name="iopSearchCheckAll" id="iopSearchCheckAll"></th><th style="width:10px">序号</th><th style="width:200px">策略配置</th><th style="width:10px">操作</th></tr>
										</thead>
										<tbody id="tableTbodyByIdYa">`
                                    var str2=`<tbody class="zpcjEditBox">
							<tr>
								<td>
									<div class="bwSearchTablePart">
										<span>搜索</span>
										<div><span>广告位编码：</span><input type="text" placeholder="请输入广告位编码" id="adCodeBwYa"></div>
										<button type="button" style="border-radius:10px; margin-left:10px; margin-right:10px; vertical-align: unset;" class="query">查询</button>
										<button type="button" style="border-radius:10px; vertical-align: unset;" class="reset">重置</button>
									</div>
									<table id="tableByIdYa">
										<thead>
											<tr><th style="width:10px"><input type="checkbox" name="bwSearchCheckAll" id="bwSearchCheckAll"></th><th style="width:10px">序号</th><th style="width:200px">策略配置</th><th style="width:10px">操作</th></tr>
										</thead><tbody id="bwTableTbodyByIdYa">`;
                                    if(data.list && data.list.length>0){
                                        for(var i=0;i<data.list.length;i++){
                                            str +=`<tr class="">
												<td><input type="checkbox" name="iopSearchCheckThis" policyId="`+data.list[i].policyId+`"></td><td>`+(i+1)+`</td>
												<td>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>广告位编码：</p></div>
														<div><input type="text" class="myinput ggwbmYa" maxlength="40" onkeyup="this.value=this.value.replace(/[^\\w_\\,]/g,'');" value="`+data.list[i].policyGgwbm+`" placeholder="请输入广告位编码"></div>
													</div>`
                                            if(data.list[i].pointName){
                                                str+=`<div class="flexbox mt15 j_hideLink">
                                                        <div class="w10"><p class="cred">*</p><p>点位名称：</p></div>
                                                        <div><input type="text" class="myinput pointNameYa" maxlength="20" value="`+data.list[i].pointName+`" placeholder="请输入点位名称"></div>
                                                    </div>`
                                            }else {
                                                str+=`<div class="flexbox mt15 j_hideLink">
                                                        <div class="w10"><p class="cred">*</p><p>点位名称：</p></div>
                                                        <div><input type="text" class="myinput pointNameYa" maxlength="20" value="" placeholder="请输入点位名称"></div>
                                                    </div>`
                                            }
													str+=`<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>策划编码：</p></div>
														<div><input type="text" class="myinput chbmYa" maxlength="40" onkeyup="this.value=this.value.replace(/[^\\w_\\,]/g,'');" value="`+data.list[i].planId+`" placeholder="请输入策划编码"></div>
													</div>
													<div class="flexbox align-s mt15">
														<div class="w10"><p class="cred">*</p><p>图片：</p></div>
														<div class="flex1 uploadImgBox">
															<div class="flexbox"><div class="selectImg"><button class="uploadBtn">浏览</button><input type="hidden"/><input type="file" accept="image/png,image/jpeg" value="" class="file myinput"></div></div>
															<div class="textLeftYa colorRedYa">注:建议上传199*221像素，大小不超过100k，格式为jpg，png的图片</div>
															<div class="shownamebox j_name textLeftYa" style="display: block;"><p class="name">`+vm.path+data.list[i].imageName+`</p><p class="del j_del">删除</p></div>
															<p class="yulanImg" style="display: block;"><img class="bgimg1" src="`+vm.path+data.list[i].imageName+`" alt=""></p>
														</div>
													</div>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>跳转地址：</p></div>
														<div style="display: flex">
															<input type="text" class="myinput jumpaddress" value="`+data.list[i].jumpAddress+`" placeholder="请输入跳转地址或选择跳转跳转广告/聚合页">
															<div class="popChooseBtnYa iopBlock" style="padding: 0 10px;">选择广告/聚合页</div>
														</div>
													</div>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>展示权重：</p></div>
														<div>
															<input type="text" class="myinput displayweights" value="`+data.list[i].displayWeights+`" maxlength="3" onkeyup="this.value=this.value.replace(/[^0-9]+?/g,'')" placeholder="请输入1-100，权重值至越大排序越靠前">
														</div>
													</div>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>有效时间：</p></div>
														<div class="inputWidthYa">
															<input id='startTime1' name='startTime1' value="`+data.list[i].validTimeBegin+`" placeholder="请输入开始时间" readonly='readonly' class='Wdate w150 vaildtimebegin' onclick="WdatePicker({maxDate:'#F{$dp.$D(\\\'endTime1\\\')}',dateFmt:'yyyy-MM-dd'})"/>
															 ~ 
															<input id='endTime1' name='endTime1' value="`+data.list[i].validtimeEnd+`" placeholder="请输入结束时间" readonly='readonly' class='Wdate w150 vaildtimeeng' onClick="WdatePicker({minDate:'#F{$dp.$D(\\\'startTime1\\\')}',dateFmt:'yyyy-MM-dd'})"/>
														</div>
													</div>
												</td>
												<td class="singleSave">保存</td>
											</tr>`
                                        }
                                    }else{
                                        var goodcodeinput="IOPAD" + new Date().getTime();
                                        str+=`<tr class="">
												...
												<td>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>广告位编码：</p></div>
														<div><input type="text" class="myinput ggwbmYa" maxlength="40" onkeyup="this.value=this.value.replace(/[^\\w_\\,]/g,'');" value="`+goodcodeinput+`" placeholder="请输入广告位编码"></div>
													</div>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>点位名称：</p></div>
														<div><input type="text" class="myinput pointNameYa" maxlength="20" value="" placeholder="请输入点位名称"></div>
													...
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>策划编码：</p></div>
														<div><input type="text" class="myinput chbmYa" maxlength="40" onkeyup="this.value=this.value.replace(/[^\\w_\\,]/g,'');" value="" placeholder="请输入策划编码"></div>
													</div>
													<div class="flexbox align-s mt15">
														<div class="w10"><p class="cred">*</p><p>图片：</p>
														</div>
														<div class="flex1 uploadImgBox">
															<div class="flexbox">
																<div class="selectImg"><button class="uploadBtn">浏览</button><input type="hidden"/><input type="file" accept="image/png,image/jpeg" value="" class="file myinput"></div>
															...
															<div class="textLeftYa colorRedYa">注:建议上传199*221像素，大小不超过100k，格式为jpg，png的图片</div>
															<div class="shownamebox j_name textLeftYa"><p class="name"></p><p class="del j_del">删除</p></div>
															<p class="yulanImg"><img class="bgimg1" src="" alt=""></p>
														</div>
													</div>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>跳转地址：</p></div>
														<div style="display: flex">
															<input type="text" class="myinput jumpaddress" value="" placeholder="请输入跳转地址或选择跳转跳转广告/聚合页">
															<div class="popChooseBtnYa iopBlock" style="padding: 0 10px;">选择广告/聚合页</div>
														</div>
													</div>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>展示权重：</p></div>
														<div>
															<input type="text" class="myinput displayweights" value="" maxlength="3" onkeyup="this.value=this.value.replace(/[^0-9]+?/g,'')" placeholder="请输入1-100，权重值至越大排序越靠前">
														</div>
													</div>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>有效时间：</p></div>
														<div class="inputWidthYa">
															<input id='startTime1' name='startTime1' placeholder="请输入开始时间" readonly='readonly' class='Wdate w150 vaildtimebegin' onclick="WdatePicker({maxDate:'#F{$dp.$D(\\\'endTime1\\\')}',dateFmt:'yyyy-MM-dd'})" value='' />
															 ~ 
															<input id='endTime1' name='endTime1' placeholder="请输入结束时间" readonly='readonly' class='Wdate w150 vaildtimeeng' onClick="WdatePicker({minDate:'#F{$dp.$D(\\\'startTime1\\\')}',dateFmt:'yyyy-MM-dd'})" value='' />
														</div>
													</div>
												</td>
												<td class="singleSave">保存</td>
                                    ...
                                    if(data.listBw && data.listBw.length>0){
                                        for(var i=0;i<data.listBw.length;i++){
                                            str2+=`<tr class="">
												<td><input type="checkbox" name="bwSearchCheckThis" policyId="`+data.listBw[i].policyId+`"></td><td>`+(i+1)+`</td>
												<td>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>广告位编码：</p></div>
														<div><input type="text" class="myinput ggwbmYa" maxlength="40" onkeyup="this.value=this.value.replace(/[^\\w_\\,]/g,'');" value="`+data.listBw[i].policyGgwbm+`" placeholder="请输入广告位编码"></div>
													</div>`
                                            if(data.listBw[i].pointName){
                                                str2+=`<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>点位名称：</p></div>
														<div><input type="text" class="myinput pointNameYa" maxlength="20" value="`+data.listBw[i].pointName+`" placeholder="请输入点位名称"></div>
													</div>`
                                            }else {
                                                str2+=`<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>点位名称：</p></div>
													...
													str2+=`<div class="flexbox align-s mt15">
														<div class="w10"><p class="cred">*</p><p>图片：</p>
														</div>
														<div class="flex1 uploadImgBox">
															<div class="flexbox">
															...
															<div class="textLeftYa colorRedYa">注:建议上传199*221像素，大小不超过100k，格式为jpg，png的图片</div>
															<div class="shownamebox j_name textLeftYa" style="display: block;"><p class="name">`+vm.path+data.listBw[i].imageName+`</p><p class="del j_del">删除</p></div>
															<p class="yulanImg" style="display: block;"><img class="bgimg1" src="`+vm.path+data.listBw[i].imageName+`" alt=""></p>
														</div>
													</div>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>跳转地址：</p></div>
														<div style="display: flex">
															<input type="text" class="myinput jumpaddress" value="`+data.listBw[i].jumpAddress+`" placeholder="请输入跳转地址或选择跳转跳转广告/聚合页">
															<div class="popChooseBtnYa iopBlock" style="padding: 0 10px;">选择广告/聚合页</div>
														</div>
													</div>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>展示权重：</p></div>
														<div>
															<input type="text" class="myinput displayweights" value="`+data.listBw[i].displayWeights+`" maxlength="3" onkeyup="this.value=this.value.replace(/[^0-9]+?/g,'')" placeholder="请输入1-100，权重值至越大排序越靠前">
														</div>
													</div>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>有效时间：</p></div>
														<div class="inputWidthYa">
															<input id='startTime2' name='startTime2' value="`+data.listBw[i].validTimeBegin+`" placeholder="请输入开始时间" readonly='readonly' class='Wdate w150 vaildtimebegin' onclick="WdatePicker({maxDate:'#F{$dp.$D(\\\'endTime2\\\')}',dateFmt:'yyyy-MM-dd'})"/>
															 ~ 
															<input id='endTime2' name='endTime2' value="`+data.listBw[i].validtimeEnd+`" placeholder="请输入结束时间" readonly='readonly' class='Wdate w150 vaildtimeeng' onClick="WdatePicker({minDate:'#F{$dp.$D(\\\'startTime2\\\')}',dateFmt:'yyyy-MM-dd'})"/>
														</div>
													</div>
												</td>
												<td class="singleSave">保存</td>
                                        ...
                                        var goodcodeinput="IOPADBW" + new Date().getTime();
                                        str2+=`<tr class="">
												<td><input type="checkbox" name="bwSearchCheckThis"></td><td>1</td>
												<td>
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>广告位编码：</p></div>
														<div><input type="text" class="myinput ggwbmYa" maxlength="40" onkeyup="this.value=this.value.replace(/[^\\w_\\,]/g,'');" value="`+goodcodeinput+`" placeholder="请输入广告位编码"></div>
													...
													<div class="flexbox mt15 j_hideLink">
														<div class="w10"><p class="cred">*</p><p>有效时间：</p></div>
														<div class="inputWidthYa">
															<input id='startTime2' name='startTime2' placeholder="请输入开始时间" readonly='readonly' class='Wdate w150 vaildtimebegin' onclick="WdatePicker({maxDate:'#F{$dp.$D(\\\'endTime2\\\')}',dateFmt:'yyyy-MM-dd'})" value='' />
															 ~ 
															<input id='endTime2' name='endTime2' placeholder="请输入结束时间" readonly='readonly' class='Wdate w150 vaildtimeeng' onClick="WdatePicker({minDate:'#F{$dp.$D(\\\'startTime2\\\')}',dateFmt:'yyyy-MM-dd'})" value='' />
														</div>
													</div>
												</td>
												<td class="singleSave">保存</td>
                                    ...
                                    str2+=`</tbody></table></td></tr></tbody>`;
                                    $('.modals #htmlCon table').css({'margin-top':'0'}).html('').append(str);
                                    $('.modals #htmlCon2').html('').append('<table width="100%" style=\'margin-top: 0rem;margin-bottom: 0.5rem;\'>'+str2+"</table>");
                                    function rgbToHex(str){//rgb转hex
                                        let reg = /^(rgb|RGB)/;if(!reg.test(str)){return;}
                                    ...
                                    if(!searchBk){
                                        searchBk = "#FFFFFF";
                                        $(".backColorInputIopYa").val(searchBk);
                                        $(".backColorInputIopYa").next().val(searchBk);
                                    }else {
                                        $(".backColorInputIopYa").val(rgbToHex(searchBk));
                                        $(".backColorInputIopYa").next().val(rgbToHex(searchBk));
                                    }
                                    if(!searchBorder){
                                        searchBorder = "#518BFF";
                                        $(".borderColorInputIopYa").val(searchBorder);
                                        $(".borderColorInputIopYa").next().val(searchBorder);
                                    }else {
                                        $(".borderColorInputIopYa").val(rgbToHex(searchBorder));
                                        $(".borderColorInputIopYa").next().val(rgbToHex(searchBorder));
                                    }
                                }else {
                                    alert(data.msg);
                                    fadeOut();
                    ...
                    function xh(saveTarget) { //序号
                        var tableLine = saveTarget.find("#tableByIdYa").get(0);
                        for (var i = 0; i < tableLine.rows.length; i++) {if(i>0){tableLine.rows[i].cells[1].innerHTML = (i);}}
                    }
                    ...
                    $('#bwSearchCheckAll').change(function(){ //全选
                        if($(this).prop("checked")){// 处理选中状态
                            $('input[name=bwSearchCheckThis]').prop('checked', true);
                        }else{// 处理未选中状态
                            $('input[name=bwSearchCheckThis]').prop('checked', false);
                        }
                    });
                    //删除图片
                    $(document).on("click",".j_del",function(){
                    ...
                    $(document).off('change',"input[name=bwSearchCheckThis]").on('change','input[name=bwSearchCheckThis]',function(){ //反选
                        if($(this).prop("checked")){// 处理选中状态
                            var numIop = 0
                            $('input[name=bwSearchCheckThis]').each(function() {
                                if($(this).prop("checked")){numIop++}
                            })
                            if(numIop == $('input[name=bwSearchCheckThis]').length){
                                $('#bwSearchCheckAll').prop('checked', true);
                            }
                        }else{
                            $('#bwSearchCheckAll').prop('checked', false);
                        }
                    });
                    $(document).off('click',".iopDeleteYa").on('click','.iopDeleteYa',function() { //删除
                        var tab,saveTarget;
                        layer.find('.selectLi').find('li').each(function (i,v) {
                            if($(v).hasClass('cur') && i==0){tab = i;saveTarget=layer.find("#htmlCon");}
                            if($(v).hasClass('cur') && i==1){tab = i;saveTarget=layer.find("#htmlCon2");}
                        });
                        var ids = [];
                        if(tab==1){
                            $('input[name=bwSearchCheckThis]:checked').each(function() {if($(this).attr("policyId")){ids.push($(this).attr("policyId"))}})
                        }else {
                            $('input[name=iopSearchCheckThis]:checked').each(function() {if($(this).attr("policyId")){ids.push($(this).attr("policyId"))}})
                        }
                        if(ids.length<=0){
                            alert('请先勾选要删除的项');
                            return ;
                        ...
                        $.ajax({
                            url:vm.path + '/tWlmDecorate/deletePolicyByIds.do', type:'post', data:{ids:ids.toString()},
                            success:function (data) {
                                console.log(data);
                                if(data.code =="suc"){
                                    alert('删除成功')
                                    //执行页面数据删除
                                    if(tab==1){
                                        $('input[name=bwSearchCheckThis]:checked').each(function() {$(this).parent().parent().remove()})
                                    }else {
                                        $('input[name=iopSearchCheckThis]:checked').each(function() {$(this).parent().parent().remove()})
                                    }
                                    xh(saveTarget)
                                }else {alert(data.msg)}
                            },
                    ...
                    $(document).off('click',".newIopBtnYa .iopAddYa").on('click','.newIopBtnYa .iopAddYa',function() { //添加
                        var saveTarget,tab,tagId,goodcodeinput;
                        layer.find('.selectLi').find('li').each(function (i,v) {
                            if($(v).hasClass('cur') && i==0){tab = i;saveTarget=layer.find("#htmlCon");tagId="tableTbodyByIdYa";}
                            if($(v).hasClass('cur') && i==1){tab = i;saveTarget=layer.find("#htmlCon2");tagId="bwTableTbodyByIdYa";}
                        });
                        if(saveTarget.find('#'+tagId+' tr').length == 21){alert('最多21条');return;}
                        var randomYa = new Date().getTime()
                        var sTimeYa = "startTime"+randomYa
                        var eTimeYa = "endTime"+randomYa+1
                        var checkFlagYa = '',bwCheckFlagYa = '';
                        if(saveTarget.find('#iopSearchCheckAll').prop("checked")){// 处理选中状态
                            checkFlagYa = 'checked'
                        }
                        if(saveTarget.find('#bwSearchCheckAll').prop("checked")){// 处理选中状态
                            bwCheckFlagYa = 'checked'
                        }
                        if(tab == 0){
                            goodcodeinput = "IOPAD" + new Date().getTime();
                        }else{
                            goodcodeinput = "IOPADBW" + new Date().getTime();
                        }
                        var trs = `<tr class="">`
                        if(tab==1){
                            trs+=`<td><input type="checkbox" name="bwSearchCheckThis" ${bwCheckFlagYa}></td>`
                        }else {
                            trs+=`<td><input type="checkbox" name="iopSearchCheckThis" ${checkFlagYa}></td>`
                        }
								trs+=`<td></td>
								<td>
									<div class="flexbox mt15 j_hideLink">
                        ...
                        if(tab == 0){
                            trs +='<div class="flexbox mt15 j_hideLink">' +
                                '<div class="w10"><p class="cred">*</p><p>策划编码：</p></div>' +
                                '<div><input type="text" class="myinput chbmYa" maxlength="40" onkeyup="this.value=this.value.replace(/[^\\w_\\,]/g,\'\');" value="" placeholder="请输入策划编码"></div>' +
                                '</div>'
                        }
									trs+=`<div class="flexbox align-s mt15">
										<div class="w10"><p class="cred">*</p><p>图片：</p></div>
										<div class="flex1 uploadImgBox">
											<div class="flexbox">
												<div class="selectImg"><button class="uploadBtn">浏览</button><input type="hidden"/><input type="file" accept="image/png,image/jpeg" value="" class="file myinput"></div>
									...
									<div class="flexbox mt15 j_hideLink">
										<div class="w10"><p class="cred">*</p><p>有效时间：</p></div>
										<div class="inputWidthYa">
											<input id='${sTimeYa}' name='${sTimeYa}' readonly='readonly' placeholder="请输入开始时间" class='Wdate w150 vaildtimebegin' onclick="WdatePicker({maxDate:'#F{$dp.$D(\\\'${eTimeYa}\\\')}',dateFmt:'yyyy-MM-dd'})" value='' />
											~ 
											<input id='${eTimeYa}' name='${eTimeYa}' readonly='readonly' placeholder="请输入结束时间" class='Wdate w150 vaildtimeeng' onclick="WdatePicker({minDate:'#F{$dp.$D(\\\'${sTimeYa}\\\')}',dateFmt:'yyyy-MM-dd'})" value='' />
										</div>
									</div>
								</td>`
                        if(tab == 0){
                            trs+=`<td class="singleSave">保存</td>`
                        }else{
                            trs+=`<td class="singleSave">保存</td>`
                        }
							trs+=`</tr>`
                        saveTarget.find('#'+tagId).append(trs)
                        xh(saveTarget)
                    })
                    $(document).off('click',".iopSearchTablePart>button[class='query']").on('click','.iopSearchTablePart>button[class="query"]',function() {//搜索
                        var adCodeIopYa = $('#adCodeIopYa').val(),chCodeIopYa = $('#chCodeIopYa').val()
                        if(chCodeIopYa=='' && adCodeIopYa ==''){
                            $('#tableTbodyByIdYa tr').each(function() {$(this).removeClass('none')})
                        }else {
                            $('#tableTbodyByIdYa tr').each(function() {
                                $(this).addClass('none')
                                if($(this).find('.ggwbmYa').val() != '' && $(this).find('.ggwbmYa').val().indexOf(adCodeIopYa) != -1 && adCodeIopYa!=''){$(this).removeClass('none')}
                                if($(this).find('.chbmYa').val() != '' && $(this).find('.chbmYa').val().indexOf(chCodeIopYa) != -1 && chCodeIopYa!=''){$(this).removeClass('none')}
                            })
                        }
                    })
                    $(document).off('click',".iopSearchTablePart>button[class='reset']").on('click','.iopSearchTablePart>button[class="reset"]',function() {//重置
                        $('#adCodeIopYa').val('');$('#chCodeIopYa').val('')
                        $('#tableTbodyByIdYa tr').each(function() {$(this).removeClass('none')})
                    })
                    $(document).off('click',".bwSearchTablePart>button[class='query']").on('click','.bwSearchTablePart>button[class="query"]',function() {//搜索
                        var adCodeIopYa = $('#adCodeBwYa').val()
                        $('#bwTableTbodyByIdYa tr').each(function() {
                            $(this).addClass('none')
                            if($(this).find('.ggwbmYa').val()!=''&&$(this).find('.ggwbmYa').val().indexOf(adCodeIopYa) != -1){$(this).removeClass('none')}
                        })
                    })
                    $(document).off('click',".bwSearchTablePart>button[class='reset']").on('click','.bwSearchTablePart>button[class="reset"]',function() {//重置
                        $('#adCodeBwYa').val('')
                        $('#bwTableTbodyByIdYa tr').each(function() {$(this).removeClass('none')})
                    })
                    $(document).off('click',".iopSaveYa").on('click','.iopSaveYa',function() {//保存
                        var saveTarget,tab,tagId;
                        layer.find('.selectLi').find('li').each(function (i,v) {
                            if($(v).hasClass('cur') && i==0){tab = i;}
                            if($(v).hasClass('cur') && i==1){tab = i;}
                        });
                        var boxid = new Date().getTime();boxid = "box"+boxid;
                        var searchImg = $('.iopSearchBtnImgName1').text();
                        var iopadgoodcode = $("#iopadgoodcode").val();
                        var iopadgoodcodename = $("#iopadgoodcodename").val();
                        if($('input[name=iopFlagYa]:checked').val() == '0'){
                            p.find('.iopAdSearch').remove();
                        }else if($('input[name=iopFlagYa]:checked').val() == '1'){//需要搜索框
                            if(!searchImg){alert('请上传搜索图片');return ;}
                            if(!search){//没有搜索标签时重新补充标签
                                var html = '<div class="iopAdSearch editing" id="'+boxid+'" ><div style="background: rgb(255, 255, 255); border-color: rgb(81, 139, 255);">' +
                                    '<img class="iopAdSearIcon" src="../../images/decorate/iopAdvertisement/2.png" alt="">' +
                                    '<input type="text" maxlength="11" name="phone" pattern="[0-9]*" readonly="" onkeyup="value=value.replace(/[^\\d]/g,\'\')" ' +
                                    'onblur="value=value.replace(/[^\\d]/g,\'\')" placeholder="输入手机号查询">';
                                if(iopadgoodcode){
                                    html += '<img class="iopAdSearBtn goodcodeinput" goodcodeinputsign="goodcodeinputsign" goodcodeinput="'+iopadgoodcode+'" goodcodeinputname="'+iopadgoodcodename+'" src="'+searchImg+'" alt="">';
                                }else {
                                    html += '<img class="iopAdSearBtn" src="'+searchImg+'" alt="">';
                                }
                                html += '</div></div>';
                                p.find('.iopAdSwiperBox').before(html);
                            }else {
                                if(iopadgoodcode){
                                    p.find(".iopAdSearBtn").attr("goodcodeinputsign","goodcodeinputsign").attr("goodcodeinput",iopadgoodcode).attr("goodcodeinputname",iopadgoodcodename).addClass("goodcodeinput");
                                }
                            }
                        }
                        p.find('.iopAdSearch>div').css({'background':$('#backColorIopYa').val(),'border-color':$('#borderColorIopYa').val()})
                        var bkImg = $('.iopBackImgName1').text();
                        if(bkImg){
                            p.find('.iopAdvertisementYa').attr("style","background-image:url("+bkImg+");");
                        }else {
                            alert('请上传背景图片');
                            return ;
                        }
                        var iopList=[], bwList=[],addFlag = true, msgNumMap = new Map(),msgPlanMap = new Map(),msglineMap = new Map();
                        if(tab==0){
                            $('#tableTbodyByIdYa tr').each(function(i) {
                                var iopMap = {};
                                if(!$(this).find('.ggwbmYa').val()){alert('请输入第'+(i+1)+'项入广告位编码');addFlag=false;return false;}
                                iopMap.policyGgwbm = $(this).find('.ggwbmYa').val();
                                if(!$(this).find('.pointNameYa').val()){alert('请输入第'+(i+1)+'项入点位名称');addFlag=false;return false;}
                                iopMap.pointName = $(this).find('.pointNameYa').val();
                                if(!$(this).find('.chbmYa').val()){alert('请输入第'+(i+1)+'项入策划编码');addFlag=false;return false;}
                                iopMap.planId = $(this).find('.chbmYa').val();
                                if(!$(this).find('.name').text()){alert('请上传第'+(i+1)+'项图片');addFlag=false;return false;}
                                iopMap.imageName = $(this).find('.name').text();
                                iopMap.imageName = iopMap.imageName.substr(iopMap.imageName.indexOf('/images'),iopMap.imageName.length)
                                if(!$(this).find('.jumpaddress').val()){alert('请输入第'+(i+1)+'项跳转地址');addFlag=false;return false;}
                                iopMap.jumpAddress = $(this).find('.jumpaddress').val();//跳转地址
                                if(!$(this).find('.displayweights').val()){alert('请输入第'+(i+1)+'项权重');addFlag=false;return false;}
                                if(parseInt($(this).find('.displayweights').val())>100){alert('第'+(i+1)+'项权重值不正确');addFlag=false;return false;}
                                iopMap.displayWeights = $(this).find('.displayweights').val();//权重
                                if(!$(this).find('.vaildtimebegin').val()){alert('请输入第'+(i+1)+'项有效时间开始时间');addFlag=false;return false;}
                                iopMap.validTimeBegin = $(this).find('.vaildtimebegin').val();
                                if(!$(this).find('.vaildtimeeng').val()){alert('请输入第'+(i+1)+'项有效时间结束时间');addFlag=false;return false;}
                                iopMap.validtimeEnd = $(this).find('.vaildtimeeng').val();
                                if($(this).find("input[name='iopSearchCheckThis']").attr("policyId")){
                                    iopMap.policyId=$(this).find("input[name='iopSearchCheckThis']").attr("policyId");
                                }
                                iopList.push(iopMap);
                                msgNumMap.set(i+1,iopMap.policyGgwbm);msgPlanMap.set(i+1,iopMap.planId);
                            })
                        }else {
                            $('#bwTableTbodyByIdYa tr').each(function(i) {
                                var bwMap = {};
                                if(!$(this).find('.ggwbmYa').val()){alert('请输入第'+(i+1)+'项入广告位编码');addFlag=false;return false;}
                                bwMap.policyGgwbm = $(this).find('.ggwbmYa').val();
                                if(!$(this).find('.pointNameYa').val()){alert('请输入第'+(i+1)+'项入点位名称');addFlag=false;return false;}
                                bwMap.pointName = $(this).find('.pointNameYa').val();
                                if(!$(this).find('.name').text()){alert('请上传第'+(i+1)+'项图片');addFlag=false;return false;}
                                bwMap.imageName = $(this).find('.name').text();
                                bwMap.imageName = bwMap.imageName.substr(bwMap.imageName.indexOf('/images'),bwMap.imageName.length)
                                if(!$(this).find('.jumpaddress').val()){alert('请输入第'+(i+1)+'项跳转地址');addFlag=false;return false;}
                                bwMap.jumpAddress = $(this).find('.jumpaddress').val();//跳转地址
                                if(!$(this).find('.displayweights').val()){alert('请输入第'+(i+1)+'项权重');addFlag=false;return false;}
                                if(parseInt($(this).find('.displayweights').val())>100){alert('第'+(i+1)+'项权重值不正确');addFlag=false;return false;}
                                bwMap.displayWeights = $(this).find('.displayweights').val();//权重
                                if(!$(this).find('.vaildtimebegin').val()){alert('请输入第'+(i+1)+'项有效时间开始时间');addFlag=false;return false;}
                                bwMap.validTimeBegin = $(this).find('.vaildtimebegin').val();
                                if(!$(this).find('.vaildtimeeng').val()){alert('请输入第'+(i+1)+'项有效时间结束时间');addFlag=false;return false;}
                                bwMap.validtimeEnd = $(this).find('.vaildtimeeng').val();
                                if($(this).find("input[name='bwSearchCheckThis']").attr("policyId")){
                                    bwMap.policyId=$(this).find("input[name='bwSearchCheckThis']").attr("policyId");
                                }
                                bwList.push(bwMap);
                                msgPlanMap.set(i+1,bwMap.policyGgwbm);
                            })
                        }
                        if(!addFlag){console.log('配置不完整，不进行保存');return ;}
                        var lineNum = "";//重复行
                        msgNumMap.forEach(function (v,k) {var ckCount = 0;msgNumMap.forEach(function (innv,innk) {if(innv==v){ckCount ++;}});if(ckCount>1){msglineMap.set(k,k);}});
                        msgPlanMap.forEach(function (v,k) {var ckCount = 0;msgPlanMap.forEach(function (innv,innk) {if(innv==v){ckCount ++;}});if(ckCount>1){msglineMap.set(k,k);}});
                        if(msglineMap.size>0){msglineMap.forEach(function (v,k) {lineNum+=v+",";});if(lineNum.length>0 && lineNum.indexOf(",")>-1){lineNum = lineNum.substring(0,lineNum.length-1);}tsMsg = tsMsg.replace("lineNums",lineNum.toString());alert(tsMsg);return ;}
                        $.ajax({
                            url:vm.path + '/tWlmDecorate/saveOrUpdateIopBlock.do',
                            type:'post',
                            async:false,
                            data:{decorateId:GetQueryString('id'),roomId:roomId,
                                iopList:JSON.stringify(iopList).replaceAll('"','yhzw').replace("[","").replace("]",""),
                                bwList:JSON.stringify(bwList).replaceAll('"','yhzw').replace("[","").replace("]","")},
                            success:function (data) {
                                if(data.code=='suc'){
                                    //回填id
                                    if(data.list && data.list.length>0){
                                        $('#tableTbodyByIdYa tr').each(function(i) {
                                            for(var i=0;i<data.list.length;i++){
                                                if($(this).find('.name').text().indexOf(data.list[i].imageName)>-1){$(this).find("input[name='iopSearchCheckThis']").attr("policyId",data.list[i].policyId)}
                                            }
                                        });
                                    }
                                    if(data.listBw && data.listBw.length>0){
                                        $('#bwTableTbodyByIdYa tr').each(function(i) {
                                            for(var i=0;i<data.listBw.length;i++){
                                                if($(this).find('.name').text().indexOf(data.listBw[i].imageName)>-1){$(this).find("input[name='bwSearchCheckThis']").attr("policyId",data.listBw[i].policyId)}
                                            }
                                        });
                                    }
                                    alert('保存成功');
                    ...
                    $(document).off('click',".singleSave").on('click','.singleSave',function() {
                        var tab;
                        layer.find('.selectLi').find('li').each(function (i,v) {
                            if($(v).hasClass('cur') && i==0){tab = i;saveTarget=layer.find("#htmlCon");}
                            if($(v).hasClass('cur') && i==1){tab = i;saveTarget=layer.find("#htmlCon2");}
                        ...
                        var dataMap = {};
                        dataMap.decoraterId = GetQueryString('id');
                        dataMap.decoraterRoomId = roomId;
                        if(tab == 0){
                            dataMap.policyType="0";
                            if(!$(this).prev().find('.ggwbmYa').val()){alert('请输入广告位编码');return false;}
                            dataMap.policyGgwbm = $(this).prev().find('.ggwbmYa').val();
                            if(!$(this).prev().find('.pointNameYa').val()){alert('请输入点位名称');return false;}
                            dataMap.pointName = $(this).prev().find('.pointNameYa').val();
                            if(!$(this).prev().find('.chbmYa').val()){alert('请输入入策划编码');return false;}
                            dataMap.planId = $(this).prev().find('.chbmYa').val();
                            if(!$(this).prev().find('.name').text()){alert('请上传图片');return false;}
                            dataMap.imageName = $(this).prev().find('.name').text();
                            dataMap.imageName = dataMap.imageName.substr(dataMap.imageName.indexOf('/images'),dataMap.imageName.length)
                            if(!$(this).prev().find('.jumpaddress').val()){alert('请输入跳转地址');return false;}
                            dataMap.jumpAddress = $(this).prev().find('.jumpaddress').val();//跳转地址
                            if(!$(this).prev().find('.displayweights').val()){alert('请输入权重');return false;}
                            if(parseInt($(this).prev().find('.displayweights').val())>100){alert('权重值不正确');return false;}
                            dataMap.displayWeights = $(this).prev().find('.displayweights').val();//权重
                            if(!$(this).prev().find('.vaildtimebegin').val()){alert('请输入有效时间开始时间');return false;}
                            dataMap.validTimeBegin = $(this).prev().find('.vaildtimebegin').val();
                            if(!$(this).prev().find('.vaildtimeeng').val()){alert('请输入有效时间结束时间');return false;}
                            dataMap.validtimeEnd = $(this).prev().find('.vaildtimeeng').val();
                            if($(this).prev().prev().prev().find("input[name='iopSearchCheckThis']").attr("policyId")){
                                dataMap.policyId=$(this).prev().prev().prev().find("input[name='iopSearchCheckThis']").attr("policyId");
                            }
                            var num="";
                            var thisNum = $(this).prev().prev().text();
                            $(this).parents("#tableTbodyByIdYa").find('tr').each(function (i) {
                                if(($(this).find('.ggwbmYa').val()==dataMap.policyGgwbm || $(this).find('.chbmYa').val() == dataMap.planId) && (i+1)!=parseInt(thisNum)){num += (i+1)+","}
                            })
                            if(num != ""){num = num.substring(0,num.length-1);alert(tsMsg.replace("lineNums",num));return ;}
                        }
                        if(tab==1){
                            dataMap.policyType="1";
                            if(!$(this).prev().find('.ggwbmYa').val()){alert('请输入广告位编码');return false;}
                            dataMap.policyGgwbm = $(this).prev().find('.ggwbmYa').val();
                            if(!$(this).prev().find('.pointNameYa').val()){alert('请输入点位名称');return false;}
                            dataMap.pointName = $(this).prev().find('.pointNameYa').val();
                            ...
                            if(!$(this).prev().find('.vaildtimeeng').val()){alert('请输入有效时间结束时间');return false;}
                            dataMap.validtimeEnd = $(this).prev().find('.vaildtimeeng').val();
                            if($(this).prev().prev().prev().find("input[name='bwSearchCheckThis']").attr("policyId")){
                                dataMap.policyId=$(this).prev().prev().prev().find("input[name='bwSearchCheckThis']").attr("policyId");
                            }
                            var num="";
                            var thisNum = $(this).prev().prev().text();
                            $(this).parents("#bwTableTbodyByIdYa").find('tr').each(function (i) {
                                if(($(this).find('.ggwbmYa').val()==dataMap.policyGgwbm) && (i+1)!=parseInt(thisNum)){num += (i+1)+","}
                            })
                            if(num != ""){num = num.substring(0,num.length-1);alert(tsMsg.replace("lineNums",num));return ;}
                        }
                        var _this = this;
                        $.ajax({
                            url:vm.path + '/tWlmDecorate/iopBlockSingleSave.do',
                            type:'post',
                            data:dataMap,
                            success:function (data) {
                                if(data.code=='suc'){
                                    $(_this).prev().prev().prev().find('input').attr("policyId",data.policyId);alert("保存成功")
                                }else {alert(data.msg);}
                            },
                            error:function () {alert("保存失败");}
                        })
        ...
        $(document).on('click','.cspz',function(e) {
            var tagName = $(this).parents('.bigFloor').find('.removeSpanClass').text();
            if (tagName == 'IOP楼层'){iopTypeName = "IOPFLOOR";}//点击的是iop楼层时，广告位编码开头串设置为iop楼层类型
            ...
            if(policy!=null){
            	...
            	for(var int=0;int<policy.length;int++){
            		...
            		var pointName = policy[int][34];
            		if(!policyGgwbm){policyGgwbm = "";}
            		...
            		tianzhi+="<tr class='clxqq' id='policypp"+int+"'>" +
            				...
            				"<div> <dl> <dt>广告位编码:</dt> <dd> <input type='text' class='policyGgwbm' name='policyGgwbm' value="+policyGgwbm+" ></dd></dl></div>"+
            				"<div> <dl> <dt>点位名称:</dt> <dd> <input type='text' maxlength='20' class='pointName' name='pointName' value="+pointName+" ></dd></dl></div>"+
            ...
            if(policyListbuwei!=null){
                ...
                var ggwbm = iopTypeName+(new Date().getTime());
            	var tianzhi2="<thead><tr><th><input type='checkbox' class='allCheckbox'></th><th>序号</th><th>策略配置</th><th>操作</th></tr></thead><tbody class='mytablebox'><tr class='clxqq'  id='policybuweipp0'><td><input type='checkbox' name='buweipp'></td><td>1</td><td>" +
                    "<div><dl><dt>广告位编码:</dt> <dd><input type=\"text\" name=\"policyGgwbm\" value='"+ggwbm+"' class=\"policyGgwbm\"></dd></dl></div>" +
                    "<div><dl><dt>点位名称:</dt> <dd><input type=\"text\" maxlength='20' name=\"pointName\" class=\"pointName\"></dd></dl></div>" +
                    "<div><dl><dt>图片:</dt><dd><div class='uploadImg normalData'><img src='../../images/images.png' alt='' class='buweiiconListpp0imagesZw'><img src='' alt='' class='buweiiconListpp0imgPerview'><p class='opacityP' id='buweiiconListpp0opacityP'>上传图片</p><input type='hidden' name='policyId' id='policyId'><input type='file' class='imgInput' onchange='upLoadImgChannel(this,0,0,2)' name='UploadBtn' id='buweiiconListpp0'><input type='hidden' name='buweiiconListppimageName' id='buweiiconListpp0imageName' ></div><span>注：建议上传350*350像素，大小不超过100k，格式为jpg，png的图片。</span></dd></dl></div>" +
                    "<div><dl><dt>跳转地址:</dt><dd><input type='hidden' name='jumpType' id='peizhijumpType'><input type='text' class='jhinp' placeholder='请输入跳转地址或选择跳转广告'  name='jumpAddress' id='jumpAddress'><span class='jhsp ad' id='jhsp0'>选择广告/聚合页</span><input type='text' readonly='readonly' name='jumpAddressStr' class='jumpAddressStrReadOnly' style='margin-left: 5px; width: 80px; float: right;border: none;'></dd></dl></div>" +
                    "<div><dl><dt>页面展示:</dt><dd class='yxsjdd'><input type='radio' name='popUpButtoniopbuwei0' class='rin' value='0'/><label for='ifyc1' class='rlb'>父页面</label><input type='radio' name='popUpButtoniopbuwei0' class='rin'  checked value='1'/><label for='ifyc2' class='rlb'>新页面</label></dd></dl></div>" +
                    "<div><dl><dt>展示权重:</dt><dd><input type='text' placeholder='请输入所占权重1~100，数值越大排位越靠前' name='displayWeights' id='displayWeights'></dd></dl></div>" +
...
editHtml += "<option value='decorateId_" + vm.aggregationList[j][0] + "' turnHref='" +  vm.aggregationList[j][2] + "?decorateId=" + vm.aggregationList[j][0] + "' bind='decorateId_" + vm.aggregationList[j][0] + "'>" + vm.aggregationList[j][1] + "</option>";
      ...
      //tab楼层配置展示
        $('.edit .demo')
            .on('mouseover',selector,function(e){
                ...
                $('body.edit .leftlableYa').css('top',$(this).offset().top)
				if($(this).hasClass('box') && $('body').hasClass('edit')){
					var lyrowName = ''
				...
				}else if($(this).hasClass('lyrow') && $('body').hasClass('edit')){
				...
				}else if($(this).hasClass('bigFloor') && $('body').hasClass('edit')){
   ...
   $('.modal,.lbmodal').on("change",".file",function(e){
       if("jpg,png,gif,jpeg,bmp".indexOf(this.files[0].name.substring(this.files[0].name.lastIndexOf('.')+1).toLowerCase())<0){
           alert("请上传图片文件");
           return ;
       }
       $(this).prev('p').text(getImageUrl(this.files[0]));
       console.log($(this).parents(".zpcjEditBox"),$(this).parents(".zpcjEditBox").length);
       if($(this).parents(".zpcjEditBox").length>0){//新版编辑弹窗图片上传

       }else{
           $(this).parents('td').find('.yulanImg img').attr("src",getImageUrl(this.files[0]));
       }
   })
                                ...
                                $.ajax({
                                    ...
                                    success: function(data){
                                        if(data.flag){
                                            ...
                                            console.log($(x).parents(".zpcjEditBox"),$(x).parents(".zpcjEditBox").length);
                                            if($(x).parents(".zpcjEditBox").length>0){//新版编辑弹窗图片上传
                                                $(x).parents(".flex1").find('.yulanImg img').attr("src",vm.path + "/" + data.imgFilePath);
                                                $(x).parents(".flex1").find(".shownamebox .name").text(vm.path + "/" + data.imgFilePath);
                                                $(x).parents(".flex1").find(".shownamebox").show();
                                                $(x).parents(".flex1").find(".yulanImg").show();
                                            }else {
    ...
    function getImageUrl(file) {
                ...
                if($('.selectP').css("display") == 'block'){// 动态广告位
                    ...
                    if(type=="head"){// 标题类型
                        ...
                        for(var i= 0 ;i<imgNum;i++){
							...
							var goodcodeinputname = $("#goodcodeinputname").val();//按钮名称
							//链接类型
                            ...
                            $(thisClickObj).find('.goodsImage a').eq(i).attr('goodcodeinputname',goodcodeinputname);//按钮名称
                            $(thisClickObj).find('.goodsImage a').eq(i).attr('goodcodeinputsign','goodcodeinputsign').addClass("goodcodeinput");
                     ...
                     */
                        ...
                        var goodsInputCode = $('#htmlCon table').find('.goodcodeinputsign').val();
                        var goodcodeinputname = $('#htmlCon table').find('#goodcodeinputname').val();
                        if(!goodsInputCode){
                            alert("请输入广告位编码！");
                            return;
                        }
                        ...
                        $(thisClickObj).find('.shopInfoConBg img').attr('goodcodeinputname',goodcodeinputname).addClass("goodcodeinput");
                        ...
                        var optsType;//操作项个数类型，1一行，2两行，3滑动，0无操作项
                        if($(".sellerInfoCss").find(".bgbox").length>=1 && $(".sellerInfoCss").find(".bgbox").length <=4){
                            optsType = 1;
                        }else if($(".sellerInfoCss").find(".bgbox").length>4 && $(".sellerInfoCss").find(".bgbox").length <=8){
                            optsType = 2;
                        }else if($(".sellerInfoCss").find(".bgbox").length>8 && $(".sellerInfoCss").find(".bgbox").length <=16){
                            optsType = 3;
                        }else if($(".sellerInfoCss").find(".bgbox").length>16){
                            alert("操作项最多16项");
                            return false;
                        }else if($(".sellerInfoCss").find(".bgbox").length<=0){
                            optsType = 0;
                        }
                        var headImg = $("#headerImg").text();
                        var bgImg = $("#bgImg").text();
                        if(!bgImg){
                            alert("请上传背景图片");
                            return false;
                        }
                        var scoregoodsinput = $("#scoregoodsinput").val();
                        var scoregoodsinputname = $("#scoregoodsinputname").val();
                        if(!scoregoodsinputname){scoregoodsinputname="";}
                        if(document.getElementById("headImg").checked){
                            if(!headImg){
                                alert("请上传头像");
                                return false;
                            }
                        }
                        var leftNums = 1;//左侧行数，小于3时调整左侧标签上下居中显示
                        if(document.getElementById("jobid").checked){leftNums ++;}
                        if(document.getElementById("groupname").checked){leftNums ++;}
                        if(!document.getElementById("headImg").checked){
                            $(thisClickObj).find(".tx").remove()
                        } else if(!$(thisClickObj).find(".tx").html()){
                            $(thisClickObj).find(".left").prepend('<div class="tx"><img src="'+headImg+'" alt=""></div>');
                        }
                        if(!document.getElementById("post").checked){
                            $(thisClickObj).find(".post").remove()
                        } else if(!$(thisClickObj).find(".post").html()){
                            $(thisClickObj).find(".left .myinfo .eye").before('<div class="post">--</div>')
                        }
                        if(!document.getElementById("jobid").checked){
                            $(thisClickObj).find(".jobid").remove()
                        } else if(!$(thisClickObj).find(".jobid").html()){
                            $(thisClickObj).find(".left .myinfo").append('<div class="jobid">--</div>')
                        }
                        if(!document.getElementById("groupname").checked){
                            $(thisClickObj).find(".groupname").remove()
                        } else if(!$(thisClickObj).find(".groupname").html()){
                            $(thisClickObj).find(".left .myinfo").append('<div class="groupname">--</div>')
                        }
                        if(!document.getElementById("scorebox").checked){
                            $(thisClickObj).find(".scorebox").remove()
                            //右上角二维码
                            if(!document.getElementById("codebox").checked){
                                $(thisClickObj).find(".codebox").remove()
                            } else if(!$(thisClickObj).find(".codebox").html()){
                                let codebox = '<div class="codebox" opttype="qrcode" onclick="funSellerInfoOpt(this);"><img class="icon" src="../../images/decorate/sellerInfo/code.png" alt=""></div>';
                                $(thisClickObj).find(".right ").append(codebox);
                            }
                        } else if(!$(thisClickObj).find(".scorebox").html()){
                            var scoreboxhtml = '<div class="scorebox goodcodeinput" goodcodeinputsign="goodcodeinputsign" goodcodeinput="'+scoregoodsinput+'" goodcodeinputname="'+scoregoodsinputname+'" ><img class="icon" src="../../images/decorate/sellerInfo/score.png" alt="">' +
                                '<span class="scoreNumb" data-encrypted="0">--</span><img class="arrow" src="../../images/decorate/sellerInfo/arrow.png" alt=""></div>';
                            $(thisClickObj).find(".right").append(scoreboxhtml);
                            //右上角二维码
                            if(!document.getElementById("codebox").checked){
                                $(thisClickObj).find(".codebox").remove()
                            } else if(!$(thisClickObj).find(".codebox").html()){
                                let codebox = '<div class="codebox" opttype="qrcode" onclick="funSellerInfoOpt(this);"><img class="icon" src="../../images/decorate/sellerInfo/code.png" alt=""></div>';
                                $(thisClickObj).find(".scorebox ").before(codebox);
                            }
                        }else if($(thisClickObj).find(".scorebox").html()){
                            //右上角二维码
                            if(!document.getElementById("codebox").checked){
                        ...
                        if($(thisClickObj).find(".scorebox").html() && scoregoodsinput){
                            $(thisClickObj).find(".scorebox").attr("goodcodeinputsign","goodcodeinputsign").attr("goodcodeinput",scoregoodsinput).attr("goodcodeinputname",scoregoodsinputname).addClass("goodcodeinput");
                        }
                        if(leftNums<3){
                            $(thisClickObj).find('.left').css('align-items','center');
                        }else {
                            $(thisClickObj).find('.left').css('align-items','flex-start');
                        }
                        console.log(leftNums);
                        $(thisClickObj).find('.sellerInfo').attr("style","background-image:url("+bgImg+");");//元素背景图
                        var opts="",opts2="";//li内容
                        let radioname,radiotext,radioimg,radiotype,addflag=true;
                        $(".sellerInfoCss").find(".bgbox").each(function (k,v) {
                            let imgSrc = $(v).find(".shownamebox").find('.name').text();//图片地址
                            if(!imgSrc){alert("请上传第"+(k+1)+"个操作项图标");addflag=false;return false;}
                            let goodscoodinput = $(v).find('input[name="goodscode"]').val();//操作类型插码编码
                            let goodscoodinputname = $(v).find('input[name="goodscodename"]').val();//操作类型点位名称
                            let btnname = $(v).find('input[name="btnname"]').val();//操作名称
                            let jumpurl = $(v).find('input[name="jumpurl"]').val();//跳转地址
                            if(!btnname){alert("请输入第"+(k+1)+"个操作项文案");addflag=false;return false;}
                            let opttype = $(v).find('select[name="opttype"]').val();//操作类型
                            if(opttype == "tourl" && !jumpurl){alert("请输入第"+(k+1)+"个操作项链接");addflag=false;return false;}
                            let isupright = $(v).find("input[type='radio']:checked").val();//是否右上角展示
                            let positionname = $(v).find("input[type='radio']:checked").attr("name");
                            if(isupright == '1'){//如果勾选了右上角则获取radio名称
                                radioname = $(v).find("input[type='radio']:checked").attr("name");//radio名称，用于右上角标识
                                radiotext = $(v).find('input[name="btnname"]').val();//操作名称，用于右上角标识
                                radioimg = $(v).find(".shownamebox").find('.name').text();//操作图片，用于右上角标识
                                radiotype = $(v).find('select[name="opttype"]').val();//操作理想，用于右上角点击响应
                            }
                            if(k>7){//超过8个存到第二个,有插码值时赋值插码标识
                                if(goodscoodinput){
                                    opts2 +=  '<li goodcodeinputsign="goodcodeinputsign" style="goodcodeinput" goodcodeinput="'+goodscoodinput+'" goodcodeinputname="'+goodscoodinputname+'" positionname="'+positionname+'" opttype="'+opttype+'" jumpurl="'+jumpurl+'"' +
                                        ' onclick="funSellerInfoOpt(this);"><img class="itemImg" src="'+imgSrc+'" alt=""><div class="itemName">'+btnname+'</div></li>';
                                }else {
                                    opts2 +=  '<li positionname="'+positionname+'" opttype="'+opttype+'" jumpurl="'+jumpurl+'" onclick="funSellerInfoOpt(this);"><img class="itemImg" src="'+imgSrc+'" alt="">' +
                                        '<div class="itemName">'+btnname+'</div></li>';
                                }
                            }else {
                                if(goodscoodinput){
                                    opts +=  '<li goodcodeinputsign="goodcodeinputsign" style="goodcodeinput" goodcodeinput="'+goodscoodinput+'" goodcodeinputname="'+goodscoodinputname+'" positionname="'+positionname+'" opttype="'+opttype+'" jumpurl="'+jumpurl+'"' +
                                        ' onclick="funSellerInfoOpt(this);"><img class="itemImg" src="'+imgSrc+'" alt=""><div class="itemName">'+btnname+'</div></li>';
                                }else {
                                    opts +=  '<li positionname="'+positionname+'" opttype="'+opttype+'" jumpurl="'+jumpurl+'" onclick="funSellerInfoOpt(this);"><img class="itemImg" src="'+imgSrc+'" alt="">' +
                                        '<div class="itemName">'+btnname+'</div></li>';
                                }
                            }
                        });
                        ...
                        //组件赋值
                        if(optsType === 0){//没有任何操作选项，移除操作部分
                            $(thisClickObj).find(".myIconlist").hide().find(".swiper-wrapper").html('');
                        }else if(optsType === 1){//一行
                            $(thisClickObj).find(".myIconlist").show().removeClass().addClass("myIconlist mySpaceBetween").find(".swiper-wrapper").html("<ul>"+opts+"</ul>");
                        }else if(optsType === 2){
                            $(thisClickObj).find(".myIconlist").show().removeClass().addClass("myIconlist myFlexwarp").find(".swiper-wrapper").html("<ul>"+opts+"</ul>");
                        }else if(optsType === 3){
                            let swiper = '<div class="swiper-slide"><ul>'+opts+'</ul></div><div class="swiper-slide"><ul>'+opts2+'</ul></div>';
                            $(thisClickObj).find(".myIconlist").show().removeClass().addClass("myIconlist mySwiperBox").find(".swiper-wrapper").html(swiper);
                            //初始化swiper
                            $(".demo .col .col .sellerInfoBox").each(function(i,u){
                                var mySwiper = new Swiper('.demo .col .col .sellerInfoBox .mySwiperBox', {
                                    pagination : '.swiper-pagination',
                                    observer: true,
                                    observeParents: true,  //监测Swiper 的祖/父元素
                                });
                        ...
                        alert("保存成功");
                    }else if(type=='shareAdvertiseCom'){//分享广告
                        var bkImg = $("#bkImg").text();
                        if(!bkImg){alert('请上传广告背景图片');return false;}
                        var adImg = $("#adImg").text();
                        if(!adImg){alert('请上传广告视频封面图片');return false;}
                        var detailImg = $("#detailImg").text();
                        if(!detailImg){alert('请上传广告详情图片');return false;}
                        var shareImg = $("#shareImg").text();
                        if(!shareImg){alert('请上传广告分享按钮图片');return false;}
                        var buyImg = $("#buyImg").text();
                        if(!buyImg){alert('请上传广告自购按钮图片');return false;}
                        var adId = $("#shareAd").val();
                        var adTitle = $("#activeTitle").val();
                        var goodcodeinputvideo = $("#shareAdVideoCode").val();//视频封面图插码值
                        var goodcodeinputnamevideo = $("#shareAdVideoCodeName").val();//视频封面图点位名称
                        var goodcodeinputdetail = $("#shareAdDetailCode").val();//广告详情图插码值
                        var goodcodeinputnamedetail = $("#shareAdDetailCodeName").val();//广告详情图点位名称
                        var goodcodeinputshare = $("#shareAdShareCode").val();//广告分享赚插码值
                        var goodcodeinputnameshare = $("#shareAdShareCodeName").val();//广告分享赚点位名称
                        var goodcodeinputbuy = $("#shareAdBuyCode").val();//广告购买省插码值
                        var goodcodeinputnamebuy = $("#shareAdBuyCodeName").val();//广告购买省点位名称
                        console.log(adId);
                        if(!adId){alert('请选择广告');return false;}
                        $(thisClickObj).find('.saBackBlock').attr("style","background-image:url("+bkImg+");");//元素背景图
                        $(thisClickObj).find('.backImgFlag').attr("src",adImg).attr("goodcodeinput",goodcodeinputvideo).attr("goodcodeinputname",goodcodeinputnamevideo).attr("goodcodeinputsign","goodcodeinputsign").addClass("goodcodeinput");//视频预览封面
                        $(thisClickObj).find('.saTopImg img').attr("src",detailImg).attr("goodcodeinput",goodcodeinputdetail).attr("goodcodeinputname",goodcodeinputnamedetail).attr("goodcodeinputsign","goodcodeinputsign").addClass("goodcodeinput");//广告详情
                        $(thisClickObj).find('.shareImgLeftYa').attr("src",shareImg).attr("adId",adId).attr('adTitle',adTitle).attr("goodcodeinput",goodcodeinputshare).attr("goodcodeinputsign","goodcodeinputsign").attr("goodcodeinputname",goodcodeinputnameshare).addClass("goodcodeinput");//广告分享赚
                        $(thisClickObj).find('.shareImgRightYa').attr("src",buyImg).attr("adId",adId).attr('adTitle',adTitle)
                            .attr('onclick',"javascript:addToProductStore.executeFvsStatcAdid('"+adId+"','1',this,'02','','1','')").attr("goodcodeinput",goodcodeinputbuy).attr("goodcodeinputsign","goodcodeinputsign").attr("goodcodeinputname",goodcodeinputnamebuy).addClass("goodcodeinput");//广告购买省
                        alert("保存成功");
                    }else if(type=='floatBlockCom'){//悬浮组件
                        var goodcodeinput = $("#activeGoodsCode").val();
                        var goodcodeinputname = $("#activeGoodsCodeName").val();
                        var popImg = $("#popImg").text();
                        if(!popImg){alert('请上传悬浮球图片');return false;}
                        var closeBtn = $("#closeBtn").text();
                        var isClose = $("input[name='closeFlag']:checked").val();
                        console.log(isClose);
                        if(isClose === '1'){
                            if(!closeBtn){alert('请上传关闭图标');return false;}
                        }
                        var adId = $("#activeTitle").val();
                        if(!adId){alert('请选择广告');return false;}
                        $(thisClickObj).find('.popFlagYa').attr("src",popImg);
                        if(isClose === '1'){
                            $(thisClickObj).find('.closeFlagYa').attr("src",closeBtn);
                            $(thisClickObj).find('.bottomImgYa').removeClass("none");
                        }else {
                            $(thisClickObj).find('.bottomImgYa').addClass("none");
                        }
                        $(thisClickObj).find('#dragDiv').attr("adid",adId).attr("goodcodeinput",goodcodeinput).attr("goodcodeinputsign","goodcodeinputsign").attr("goodcodeinputname",goodcodeinputname).addClass("goodcodeinput");
                        alert("保存成功");
                    }
                }
        ...
        $('.cspzedit-layer .savejh').click(function(e){// 聚合楼层关闭弹窗
           ...
           if (floorType && floorType == 'link_href') {
            ...
            if(floorType && floorType == 'shareAd'){
                console.log($(document).find('.shareAdBlock'));
                jumpAddress='adid='+$('input[name=jhrjhAd]:checked').val();
                jumpNameStr=$('input[name=jhrjhAd]:checked').parent().next().next().next().html();
                $(document).find('.shareAdBlock').prev().val(jumpAddress);
                $(document).find('.shareAdBlock').prev().prev().val(jumpNameStr);
            }

            if(floorType && floorType == 'popAd'){
                if(jh=="1"){
                    jumpAddress= 'adid=' + $('input[name=jhrjhAd]:checked').val();
                }else{
                    jumpAddress=$('input[name=jhr]:checked').val() + '?decorateId=' + vm.id ;
                }
                $(document).find('.popAdBlock').prev().val(jumpAddress);
            }
            if(floorType && floorType == 'iopBlockAd'){
                if(jh=="1"){
                    jumpAddress= 'adid=' + $('input[name=jhrjhAd]:checked').val();
                }else{
                    jumpAddress=$('input[name=jhr]:checked').val() + '?decorateId=' + vm.id ;
                }
                $(iopBlockObj).prev().val(jumpAddress);
            }
			...
			whyClickObj = $(this).parent().parent(),//edit父标签的父标签
			 ...
			 var editHtmlImg = "<div class='cmCon'> <table class='why_tips'><thead><tr> <th>序号</th> <th>内容配置</th> <th>操作</th> </tr></thead>"
				 ...
				 for(var i= 1 ;i<imgNum;i++){
					 ...
					 if(!newdbdh_ggwbm){newdbdh_ggwbm="BOTTOMN"+(new Date().getTime())+i}//新底部导航广告位编码不存在时，使用固定字符串+时间戳+序号生产默认插码值
					 var newdbdh_btnname = p.find('.whyview .hyNav-box li').eq(i-1).attr("goodcodeinputname");//点位名称
					 if(!newdbdh_btnname){newdbdh_btnname=""}
					...
					var	 edithyImg2 = p.find('.whyview .hyNav-box li').eq(i-1).children("a").children("div:last-child").children("img").attr("src");
					   ...
					   editqyxq="<tr class='qyqq whytr'>"
                           ...
                           +"		<dt>点位名称：</dt>"
                           +"		<dd>"
                           +" 		<input class='newdbdh_btnname' maxlength='20' value='"+newdbdh_btnname+"'>"
                           +"     </dd>"
                           +"	</dl>"
                           +"</div>"
								 +"<div class='hytddiv'>"
	...
	$('.whyedit-layer').on('click','.savewhy',function(e){
		...
		for(var i= 0 ;i<imgNum;i++){ 
			...
			var whymenuad  =  $(this).parents('.whyedit-layer').children(".whymain-content").find("table tbody").children("tr").eq(i).find(".hytddiv:nth-child(3)").find("input").val();
			var goodcodeinputname  =  $(this).parents('.whyedit-layer').children(".whymain-content").find("table tbody").children("tr").eq(i).find(".hytddiv:nth-child(2)").find("input").val();
            var newdbdh_ggwbmStr= " goodcodeinputsign='goodcodeinputsign' goodCodeInput='" + newdbdh_ggwbm + "' goodcodeinputname='"+goodcodeinputname+"' " ;
            console.log(newdbdh_ggwbmStr)
			//锦鲤
			var goldenFish  =  $(this).parents('.whyedit-layer').children(".whymain-content").find("table tbody").children("tr").eq(i).find(".hytddiv:nth-child(4)").find("select[name='goldenFish']").val();
			var programLoginFlag  =  $(this).parents('.whyedit-layer').children(".whymain-content").find("table tbody").children("tr").eq(i).find(".hytddiv:nth-child(5)").find("select[name='programLoginFlag']").val();
			console.log(goldenFish);
			var titleorname  =  $(this).parents('.whyedit-layer').children(".whymain-content").find("table tbody").children("tr").eq(i).find(".hytddiv:nth-child(3)").find("input").next().next().val();
			
			
			//获取业务图片地址
			var tpdz1 = $(this).parents('.whyedit-layer').children(".whymain-content").find("table tbody").children("tr").eq(i).find(".hytddiv:nth-child(6)").find(".wxzpic .imgPerview").attr("src");
			var tpdz2 = $(this).parents('.whyedit-layer').children(".whymain-content").find("table tbody").children("tr").eq(i).find(".hytddiv:nth-child(7)").find(".xzpic .imgPerview").attr("src");
			...
			if(whymenuad!=null&&whymenuad!=''&&whymenuad!=undefined){
			    if(whymenuad.indexOf("/rr/")>-1){
					editHxcd += "\<li class='whyli goodcodeinput' "+newdbdh_ggwbmStr+"><a onclick="+"addToProductStore.executeFvsStatcAdid(\'"+whymenuad+"&turnJumpImage="+tpdz2+"\',\'1\',this,\'02\',\'1\',null,\'"+goldenFish+"\')" +
					...
					editHxcd += "\<li class='whyli goodcodeinput' "+newdbdh_ggwbmStr+"><a onclick="+"NewBottomJumpURL(\'"+whymenuad+"\',\'"+goldenFish+"\') goldenFish='"+goldenFish+"' turnHref='"+whymenuad+"'" +
                        " titleorname='"+titleorname+"' url='"+whymenuad+"'  turnJumpImage='"+tpdz2+"' programLoginFlag='"+programLoginFlag+"'>"
				}
			}else{
				editHxcd += "\<li class='whyli goodcodeinput' "+newdbdh_ggwbmStr+"><a bindturnhrefimage='bindturnhrefimage'>"
					countS++;
			}
				if(i == 0){//自定义名称
...
$(".whyjxxz").click(function(e){
	...
	//追加后重新遍历初始化序号以及id
    var that = $(this);
    setTimeout(function () {initwhy(that,lta);},1)//一毫秒延迟
   });

   function initwhy(a,lbtth){
       var ggwbm = "BOTTOMN" + (new Date().getTime());//默认广告位编码，固定字符串+时间戳
	...
	<td>\
		<div class="hytddiv">\
			<dl>\
				<dt>广告位编码：</dt>\
				<dd>\
		 			<input class="newdbdh_ggwbm" value="'+ggwbm+'">\
				</dd>\
			</dl>\
		</div>\
		<div class="hytddiv">\
			<dl>\
				<dt>点位名称：</dt>\
				<dd>\
		 			<input maxlength=\'20\' class="newdbdh_btnname">\
				</dd>\
			</dl>\
		</div>\
		<div class="hytddiv">\
        ...
        <dt>小程序登录：</dt>\
        <dd>\
        <div class="gglogin+'+lbtth+'+"><select name="programLoginFlag"><option value="T" >是</option><option value=\'F\' selected>否</option></select></div>\
       </dd>\
            </dl>\
        </div>\
		<div class="hytddiv">\
	...
	$('.modals_cspzjhy').fadeIn(200, function() {//展示参数配置弹层
        ...
        //悬浮组件选择广告
        $(document).on('click','.popAdBlock',function(e) {
            $("#floorType").val('popAd');
            $("#tjnr2yy").hide();//隐藏应用下载
            queryPolicyPage();
            $(".tiaoType").html('选择广告/聚合页');
            ...
            $(".showpmd").hide();
            $('.modals_cspzjhy').fadeIn(200, function() {//展示参数配置弹层
                var zhi=$(this).parent().find('#activeTitle').val();
                huixianJH(zhi);
                huixanzhi=zhi;
                var layer=$('.cspzedit-layer',this);
                layer.css({
        ...
        //iop组件选择广告
        $(document).on('click','.iopBlock',function(e) {
            iopBlockObj = this;
            $("#floorType").val('iopBlockAd');
            $("#tjnr2yy").hide();//隐藏应用下载
            queryPolicyPage();
            $(".tiaoType").html('选择广告/聚合页');
            $(".pmdtext").html('');
           ...
           $(demo).on('click','.editlbt',function(e) {//deme下轮播图片弹窗加载事件
            ...
            //    idName=$(this).parent().next().children().addClass('editing').attr('id');//轮播图id获取添加editing 好像没啥用
                ...
                $('.nlbtTips').fadeIn(200, function() {
                    ...
                    console.log(p.find('.scaleSwiper').length)
                    var imgNum;//遍历
                    if(p.find('.scaleSwiper').length>0){
                        imgNum = p.find('.lbtdiv .scaleSwiper li img').length+1
                    }else{
                        imgNum = p.find('.lbtdiv .slider li img').length+1;//遍历
                    }
                    var editHtmlImg = "";
                    if(p.find('.scaleSwiper').length>0){
                        editdztj1 =  "  <div class='nrks'><div class='nrksLine'>"
                        +"<div class='swiperSclass'><span>轮播图样式：</span>"
                        +" <input type='radio' name='swiperStyle' id='swiperStyle1' value='1' class='ifglgg nninput'/><label for='swiperStyle1' class='ifdllb'>默认样式</label>"
                        +" <input type='radio' name='swiperStyle' id='swiperStyle0' value='0' class='ifglgg nninput' checked/><label for='swiperStyle0' class='ifdllb'>预显示前后图样式</label>"
                        +"      </div><span class='zysx'>注：建议图片尺寸一致</span></div>"
                        +       "<ul class='cont_list clistone' id='c_1'>";
                    }else{
                        editdztj1 =  "  <div class='nrks'><div class='nrksLine'>"
                        +"<div class='swiperSclass'><span>轮播图样式：</span>"
                        +" <input type='radio' name='swiperStyle' id='swiperStyle1' value='1' class='ifglgg nninput' checked/><label for='swiperStyle1' class='ifdllb'>默认样式</label>"
                        +" <input type='radio' name='swiperStyle' id='swiperStyle0' value='0' class='ifglgg nninput'/><label for='swiperStyle0' class='ifdllb'>预显示前后图样式</label>"
                        +"      </div><span class='zysx'>注：建议图片尺寸一致</span></div>"
                        +       "<ul class='cont_list clistone' id='c_1'>";
                    }
...
//                           
                        ...
                        goodCodeInputNewLB = p.find('.lbtdiv li a').eq(i-1).attr('goodcodeinput'),
                            goodcodeinputname = p.find('.lbtdiv li a').eq(i-1).attr('goodcodeinputname'),
                        ...
                        var radioScale = p.find('.lbtdiv').attr("dataScale")
                        if(p.find('.scaleSwiper').length>0){
                            editText = p.find('.lbtdiv .scaleSwiper li .lbtName').eq(i-1).val(),//获取轮播图标题
                            editglText = p.find('.lbtdiv .scaleSwiper li .lbtifglgg').eq(i-1).val(),//获取是否关联广告属性
//                           
                            adidhref = p.find('.lbtdiv .scaleSwiper li a').eq(i-1).find('#adidhref').val(),
                            jhyhref = p.find('.lbtdiv .scaleSwiper li a').eq(i-1).find('#jhyhref').val(),
                            yyyhref = p.find('.lbtdiv .scaleSwiper li a').eq(i-1).find('#yyyhref').val(),
                            goodCodeInputNewLB = p.find('.lbtdiv .scaleSwiper li a').eq(i-1).attr('goodcodeinput'),
                                goodcodeinputname = p.find('.lbtdiv .scaleSwiper li a').eq(i-1).attr('goodcodeinputname'),
                            titleorname = p.find('.lbtdiv .scaleSwiper li a').eq(i-1).attr('titleorname'),
                            
                            selectTypePhoneHref = p.find('.lbtdiv .scaleSwiper li a').eq(i-1).attr('selectTypePhoneHref'),
                            lbteditPicSrc = p.find('.lbtdiv .scaleSwiper li img').eq(i-1).prop('src'),//获取图片src展示
                            editPicHref = p.find('.lbtdiv .scaleSwiper li a').eq(i-1).prop('href');//图片超链接展示
                            editPicHref = p.find('.lbtdiv .scaleSwiper li a').eq(i-1).attr('href');
                        }
                        if(!goodcodeinputname){goodcodeinputname=""}
                        if(goodCodeInputNewLB==null||goodCodeInputNewLB==undefined||goodCodeInputNewLB==''){//轮播图插码值为空时使用固定字符串加时间戳加序号形式生产默认插码值
                            goodCodeInputNewLB="NEWLB"+(new Date().getTime())+i;
                        }
                        if(!titleorname){
                            titleorname = '';
                        }
                        ...
                        var editdztj = "<li class='lbt_li'>"
                                    ...
                                    editdztj+="<tr>"
                                        ...
                                        +"              </tr>";
                        editdztj+="<tr><th>点位名称：</th><td><input class='inputtxt' id='goodcodeinputname' maxlength='20' name='' type='text' value="+goodcodeinputname+"></td></tr>";
                                if(i==1){
                                        editdztj += "   <tr>"
                                        +"                  <th></th>"
                                        +"                  <td  class='ydtd'>"
           ...
           /********************************* 关联广告 end ***************************************** */
            ...
            $('.lbmodal').on('click','.lbt-edit button',function(e){// 点击保存
                ...
                var editLbt='';
                var swiperType = $(this).parents('.lbmodal').children(".lbtdes").find(".swiperSclass").find('input[name="swiperStyle"]:checked').val()//swiper样式类型0为预显示  1默认
                for(var i= 0 ;i<imgNum;i++){ 
                  ...
                  //  var ifgg = $(this).parents('.lbmodal').children(".lbtdes").find(".cont_list li").find(".ifglgg[name='ifglgg"+(i+1)+"']:checked").val();//是否关联广告
                    ...
                    var goodcodeinputname = $(this).parents('.lbmodal').children(".lbtdes").find(".cont_list li").find('#goodcodeinputname').eq(i).val();// 点位名称
                    if(!goodcodeinputname){goodcodeinputname=""}
                    ...
                    if(ifgg == '1'){//ywinput
                        ...
                        if (ggHref.indexOf('adid=') > -1) {
                          ...
                          ggHref = ggHref + '&shopId=';
                            ...
                            editLbt += ' <a style="goodcodeinputsign" href="javascript:void(0)" '+click+' goodcodeinputsign="goodcodeinputsign" goodcodeinputname="'+goodcodeinputname+'" goodcodeinput='+ggbianma+' titleorname='+ggHrefName+'><img src="'+imgSrc+'" alt=""><input type="hidden" id="jhyhref" value="'+ggHref+'"><input type="hidden" value="'+btnm+'" class="lbtName"/><input type="hidden" value="'+ifgg+'" class="lbtifglgg"/></a>';
                        }else if (ggHref.indexOf('pkid=') > -1){
                       	 	var click = ' onclick=\"javascript:addToProductStore.executeFvsStatcAdid('+ '\'' + ggHref + '\'' + ',\'1\',this,\'02\',\'\',\'1\')\"';
                       	 	editLbt += ' <a style="goodcodeinputsign" href="javascript:void(0)" bindturnhrefimage="bindturnhrefimage" goodcodeinputname="'+goodcodeinputname+'" titleorname='+ggHrefName+' class="" url="'+ggHref+'" '+click+' goodcodeinputsign="goodcodeinputsign" goodcodeinput='+ggbianma+'><img src="'+imgSrc+'" alt=""><input type="hidden" id="yyyhref" value="'+ggHref+'"><input type="hidden" value="'+btnm+'" class="lbtName"/><input type="hidden" value="'+ifgg+'" class="lbtifglgg"/></a>';
                        }
                    } else {
                        if (!imgHref) {
                        ...
                        editLbt += '<a style="goodcodeinputsign" href="'+imgHref+'" url="'+imgHref+'" goodcodeinputname="'+goodcodeinputname+'" goodcodeinputsign="goodcodeinputsign" goodcodeinput='+ggbianma+'>\
                ...
                if(swiperType == '0'){
                    $(lbtthisClickObj).find('.lbtdiv').append('<div class="scaleSwiper"><ul class="swiper-wrapper" id="">'+editLbt+'</ul><div class="swiper-pagination"></div></div>');
                    $(lbtthisClickObj).find('.lbtdiv .Gallery').css({'display':'none'})
                    console.log($(lbtthisClickObj).find('.lbtdiv .Gallery').length)
                    swiperScale = new Swiper('.scaleSwiper', {
						// autoplay: 3000,//可选选项，自动滑动
						slidesPerView: 1.3, //头部一行显示多少个 .5表示显示半个
						pagination : '.swiper-pagination',
						loop: false, //是否可循环
						centeredSlides: true,
						observer: true, 
                    ...
                    $(lbtthisClickObj).find('.lbtdiv .Gallery').css({'display':'block'})
                    $('.scaleSwiper').remove()
                    swiperScale = null
                    $(lbtthisClickObj).find('.lbtdiv .slider').removeClass('none')
                    $(lbtthisClickObj).find('.lbtdiv .imgs-container').html(editLbt);
                    reSlide();
                }
                $(lbtthisClickObj).find('.lbtdiv').attr('dataScale',swiperType)
                sizeInit();
...
function showJtIop(type){
    var ggwbm = iopTypeName+(new Date().getTime());
    if(type=="0"){
        var tianzhi2="<thead><tr><th><input type='checkbox' class='allCheckbox'></th><th>序号</th><th>策略配置</th><th>操作</th></tr></thead><tbody class='mytablebox'><tr class='clxqq' id='policypp0'><td><input type='checkbox' name='ioppp'></td><td id='xuhaoCount'>1</td><td>" +
            "<div><dl><dt>广告位编码:</dt> <dd><input type=\"text\" name=\"policyGgwbm\" value='"+ggwbm+"' class=\"policyGgwbm\"></dd></dl></div>" +
            "<div><dl><dt>点位名称:</dt> <dd><input type=\"text\" maxlength='20' name=\"pointName\" class=\"pointName\"></dd></dl></div>" +
            "<div><dl><dt>策划ID:</dt><dd><input type='hidden' name='policyId' id='policyId'><input type='text' placeholder='请输入策划ID' name='planId' id='planId' autocomplete='off' onkeyup='this.value=this.value.replace(/[^\d\,]/g,'')'><span style='display: inline-block; text-align:left;width:100%;'>注：输入ID请用英文,分隔 </span></dd></dl></div>" +
            "<div><dl><dt>图片:</dt><dd><div class='uploadImg normalData'><img src='../../images/images.png' alt='' class='iconListpp0imagesZw'><img src='' alt='' class='iconListpp0imgPerview'><p class='opacityP' id='iconListpp0opacityP'>上传图片</p><input type='hidden' name='iconListppimageName' id='iconListpp0imageName' ><input type='file' class='imgInput' onchange='upLoadImgChannel(this,0,0,2)' name='UploadBtn' id='iconListpp0' ></div><span>注：建议上传350*350像素，大小不超过100k，格式为jpg，png的图片。</span></dd></dl></div>" +
            "<div><dl><dt>跳转地址:</dt><dd><input type='hidden' name='jumpType' id='peizhijumpType'><input type='text' class='jhinp' placeholder='请输入跳转地址或选择跳转广告' name='jumpAddress' id='jumpAddress' autocomplete='off'><span class='jhsp ad' id='jhsp0'>选择广告/聚合页</span><input type='text' readonly='readonly' name='jumpAddressStr' class='jumpAddressStrReadOnly' style='margin-left: 5px; width: 80px; float: right;border: none;'></dd></dl></div>" +
            "<div><dl><dt>页面展示:</dt><dd class='yxsjdd'><input type='radio' name='popUpButtoniop0' class='rin'  value='0'/><label for='ifyc1' class='rlb'>父页面</label><input type='radio' name='popUpButtoniop0' class='rin'  checked value='1'/><label for='ifyc2' class='rlb'>新页面</label></dd></dl></div>" +
            "<div><dl><dt>展示权重:</dt><dd><input type='text' placeholder='请输入所占权重1~100，数值越大排位越靠前' name='displayWeights' id='displayWeights' autocomplete='off'></dd></dl></div>" +
            "<div><dl><dt>有效时间:</dt><dd class='yxsjdd'><input id='startTimepp' name='startTimepp' readonly='readonly' class='Wdate w150'  onclick="+"WdatePicker({dateFmt:'yyyy-MM-dd'})"+" autocomplete='off'/> ~ <input id='endTimepp' name='endTimepp' readonly='readonly' class='Wdate w150' onclick="+"WdatePicker({dateFmt:'yyyy-MM-dd'})"+" /></dd></dl></div></td><td><a href='javascript:;' class='submitted saven' onclick='savePolicyFunpp(0)'>保存</a></td></tr></tbody>";
    }else{
        var tianzhi2="<thead><tr><th><input type='checkbox' class='allCheckbox'></th><th>序号</th><th>策略配置</th><th>状态</th><th>操作</th></tr></thead><tbody class='mytablebox'><tr class='clxqq' id='policyIop0'><td><input type='checkbox' name='ioppp'></td><td id='xuhaoCount'>1</td><td>" +
            "<div><dl><dt>广告位编码:</dt> <dd><input type=\"text\" id=\"policyGgwbm\" value='"+ggwbm+"' name=\"policyGgwbm\" class=\"policyGgwbm\"></dd></dl></div>" +
            "<div><dl><dt>点位名称:</dt> <dd><input type=\"text\" maxlength='20' id=\"pointName\" name=\"pointName\" class=\"pointName\"></dd></dl></div>" +
            "<div><dl><dt>策划ID:</dt><dd style='display: flex'><input type='hidden' name='policyId' id='policyId0'><input type='text' name='planId' id='planId0' autocomplete='off' readonly='readonly' class='selectLiCspzpp'><span class='jhsp jtIop' id='jhIop0'  >选择</span></dd></dl></div>" +
            "<div><dl><dt>图片:</dt><dd><div class='uploadImg normalData' readonly='readonly' class='selectLiCspzpp'><img src='' alt='' class='iconListpp0imgPerview' id='iopPath0'><input type='hidden' name='iconListppimageName' id='iopPathImg'></div></dd></dl></div>" +
            "<div><dl><dt>跳转地址:</dt><dd><input type='text' readonly='readonly' class='selectLiCspzpp' class='jhinp'  name='jumpAddress' id='jumpAddressIop0' autocomplete='off'></dd></dl></div>" +
            "<div><dl><dt>页面展示:</dt><dd class='yxsjdd'><input type='radio' name='popUpButtoniop0' class='rin' value='0'/><label for='ifyc1' class='rlb'>父页面</label><input type='radio' name='popUpButtoniop0' class='rin'  checked value='1'/><label for='ifyc2' class='rlb'>新页面</label></dd></dl></div>" +
            "<div><dl><dt>展示权重:</dt><dd><input type='text' placeholder='请输入所占权重1~100，数值越大排位越靠前' name='displayWeights' id='displayWeightsIop' autocomplete='off'></dd></dl></div>" +
            "<div><dl><dt>有效时间:</dt><dd class='yxsjdd'><input id='startTimepp0' name='startTimepp' readonly='readonly' class='selectLiCspzpp' class='Wdate w150'  autocomplete='off'/> ~ <input id='endTimepp0' name='endTimepp' readonly='readonly' class='selectLiCspzpp' class='Wdate w150' /></dd></dl></div></td><td>无</td><td><a href='javascript:;' class='submitted saven' onclick='savePolicyFunpp(0)'>保存</a></td></tr></tbody>";
    }
...
function initLbt(lbtth){ 
 var ggwbm = "NEWLB"+(new Date().getTime())+lbtth;//默认广告位编码
...
+"                   <tr>"
+"                  <th>插码：</th>"
+"                  <td><input class='inputtxt goodCodeInputNewLB' id='goodCodeInputNewLB' name='' type='text' value='"+ggwbm+"'></td>"
+"                </tr>"
    +"                   <tr>"
    +"                  <th>点位名称：</th>"
    +"                  <td><input class='inputtxt' id='lbtBtnName' maxlength='20' name='' type='text' value=''></td>"
    +"                </tr>"
+"                  <tr>"
+"                      <th></th>"
+"                      <td class='ydtd'>"
...
function savePolicyFunpp(a){
			...
			var pointName=$("#policypp"+a).find(".pointName").val();//点位名称
            var policyType=$("#policyType").val();
            ...
            if($("input[name='tjiop']:checked").val() ==='2'){
                ...
                pointName=$("#policyIop"+a).find(".ponitName").val();//插码
        ...
        $.ajax({
            ...
            data :{'policyType':policyType,'planId':planId,'jumpAddress':jumpAddress,'displayWeights':displayWeights,
                ...
                "jumpType":jumpType,"roomType":roomType,"popUpButtoniop":popUpButtoniop,'titleOrName':titleOrName,'policyGgwbm':goodcodeinputsign,pointName:pointName},
            success : function(data) {
                if(data.flag==true){
                    toast("保存成功");// 改成小黑窗
                    var tbody="<thead><tr><th><input type='checkbox' class='allCheckbox'></th><th>序号</th><th>策略配置</th><th>操作</th>	</tr></thead><tbody class='mytablebox'>";
                    	...
                    	if(policyType!='2'){
                            if(policy!=null){
                                for(var int=0;int<policy.length;int++){
                                    ...
                                    var popUpButtoniop=policy[int][21];
                                    var policyGgwbm=policy[int][23];
                                    var ponitName=policy[int][34];
                                    if(!policyGgwbm){policyGgwbm = "";}
                                    if(!ponitName){ponitName = "";}
                                    if(address==null||address==''||address==undefined){address="";}
                                    if(popUpButtoniop=="0"){
                                        ispop="<input type='radio' name='popUpButtoniop"+int+"'  class='rin' value='0' checked='checked'/><label for='ifyc1' class='rlb'  >父页面</label><input type='radio' name='popUpButtoniop"+int+"' class='rin' value='1'/><label for='ifyc2' class='rlb'  >新页面</label>";
                                    }else{
                                    ...
                                    tianzhi+="<tr class='clxqq' id='policypp"+int+"'>" +
                                        ...
                                        "<td id='xuhaoCount'>"+xuhao+"</td> <td>" +
                                        "<div> <dl> <dt>广告位编码:</dt> <dd> <input type='text' class='policyGgwbm' name='policyGgwbm' value="+policyGgwbm+"></dd></dl></div>"+
                                        "<div> <dl> <dt>点位名称:</dt> <dd> <input type='text' maxlength='20' class='ponitName' name='ponitName' value="+ponitName+"></dd></dl></div>"+
                                        "<div> <dl> <dt>策划ID:</dt> <dd> <input type='hidden' name='policyId' id='policyId' value="+policy[int][0]+"> <input type='text' placeholder='请输入策划ID' name='planId' id='planId' value="+policy[int][3]+" οnkeyup='this.value=this.value.replace(/[^\d\,]/g,'')'> <span style='display: inline-block; text-align:left;width:100%;'>注：输入ID请用英文,分隔 </span></dd></dl></div>"+
                                        "<div><dl><dt>图片:</dt><dd><div class='uploadImg normalData'><img src='../../images/images.png' alt='' class='iconListpp"+int+"imagesZw'><img src='"+vm.path+"/"+policy[int][4]+"' alt=''  class='iconListpp"+int+"imgPerview' ><p class='opacityP j-deleteImg'  id='iconListpp"+int+"opacityP' onclick="+"deleteImg('iconListpp"+int+"')>删除图片</p><input type='hidden' name='iconListppimageName' id='iconListpp"+int+"imageName' value="+policy[int][4]+"><input type='file' class='imgInput' onchange='upLoadImgChannel(this,0,0,2)' name='UploadBtn' id='iconListpp"+int+"'></div><span>注：建议上传350*350像素，大小不超过100k，格式为jpg，png的图片。</span></dd></dl></div>"+
                                        "<div><dl><dt>跳转地址:</dt><dd><input type='hidden' name='jumpType' id='peizhijumpType' value="+policy[int][18]+"><input type='text' class='jhinp' placeholder='请输入跳转地址或选择跳转广告' name='jumpAddress' id='jumpAddress' value="+address+"><span class='jhsp ad' id='jhsp"+int+"'  >选择广告/聚合页</span><input type='text' readonly='readonly' name='jumpAddressStr' class='jumpAddressStrReadOnly' style='margin-left: 5px; width: 80px; float: right;border: none;'  value='"+policy[int][22]+"'>	</dd>	</dl></div>"+
                                        "<div  ><dl><dt>页面展示:</dt><dd class='yxsjdd'>"+ispop+"</dd>	</dl></div>" +
...
function savePolicyFunbuweipp(a){
	...
	var pointName=$("#policybuweipp"+a).find(".pointName").val();//点位名称
            var policyType=$("#policyType").val();
                ...
                if(data.flag==true){
                    ...
                    jQuery.ajax({
                    	...
                    	for(var int=0;int<policy.length;int++){
                    		...
                    		tianzhi+="<tr class='clxqq'  id='policybuweipp"+int+"'><td><input type='checkbox' value="+policy[int][0]+" name='buweipp'></td><td>"+xuhao+"</td><td> " +
...
//分享广告选择广告
$(document).on('click','.shareAdBlock',function(e) {
    resetValuead();
    showggjh('ggtable');
    $(".tiaoType").html('选择广告');
    $(".pmdtext").html('广告');
    $(".showiop").hide();
    $(".showpmd").show();
    $("#floorType").val('shareAd');
    $("#tjnr1").prop('checked',true);//广告按钮选中
    $(this).parent().find('.shareAd').attr('id','shareAd')
    var zhi=$(this).parent().find('.shareAd').val();
    console.log(zhi);
    huixianJH(zhi);
    huixanzhi=zhi;
    $('.modals_cspzjhy').fadeIn(200, function() {// 展示参数配置弹层
        ...
        if($("input[name='tjiop']:checked").val() =='1'){
            ...
            $.each(ntrer,function(i,eleName){// 遍历tr
                ...
                if(i>coun){
                    ...
                    $(eleName).children("td:nth-child(3)").find(".policyGgwbm").val(iopTypeName+((new Date()).getTime()));
...
function saveBathchpp(index){//index iop推荐或补位
    ...
    var msglineMap = new Map();
	 ...
	 /*if(roomType){//房间类型 由于goodcodeinputsign未定义导致iop策略广告全部保存报错，现注释掉
		 $("#"+roomid).find("a").attr('goodCodeInput',goodcodeinputsign);
		 $("#"+roomid).find("a").attr('goodcodeinputsign','goodcodeinputsign')
	 }*/
}
        ...
        if($(this).attr('bind') != 'floorAttr_3'){ //非tab楼层
            $(this).find('.ui-draggable').each(function(){
                if($(this).hasClass("box")){
                    $(this).find("*[goodcodeinputsign='goodcodeinputsign']").each(function(u) {
            ...
            $(this).find(".smtb").each(function(u){
                ...
                $(this).find('.ui-draggable').each(function(){
                    if($(this).hasClass("box")){
                        $(this).find("*[goodcodeinputsign='goodcodeinputsign']").each(function(m) {
                            $(this).attr('data-codeInser',a+1+'-'+(u+1)+'-'+(n+1)+'-'+(m+1))
                        })
                        if($(this).find("*[goodcodeinputsign='goodcodeinputsign']").length>0){
                            n++;
                        }
                    }else if($(this).hasClass("wdg")){
                        $(this).find("*[goodcodeinputsign='goodcodeinputsign']").each(function(m) {
...
var goodsCodeMap = new Map();//获取所有的插码值以及点位名称
function setGoodsCode() {
    $(".demo").find('*[goodcodeinputsign="goodcodeinputsign"]').each(function () {
        var goodcodeinputname = $(this).attr('goodcodeinputname');
        if(!goodcodeinputname){goodcodeinputname=""}
        goodsCodeMap.set($(this).attr("data-codeinser")+"-"+$(this).attr("goodcodeinput"),goodcodeinputname);
    });
}
//iop轮播配置条件搜索 --开始
...
function sellerCardSecChange(obj) {
    if($(obj).val()=='tourl'){
        $(obj).parents(".flexbox").next().show();
    }else {
        $(obj).parents(".flexbox").next().hide();
    }
}
